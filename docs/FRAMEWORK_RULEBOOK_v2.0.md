...
| フレームワーク間依存ゼロ | フレームワーク間の直接 import / 参照を禁止。唯一の例外として ACS が定義する型・インターフェースへの参照を許可する（§4.6 参照） |
...
| **エンジン単位の自己完結** | 5 ファイル一体で自己完結する。他エンジン（フレームワーク）の存在を前提としない |
| **フレームワーク間依存ゼロ** | エンジン間の直接 import / 参照を禁止。唯一の例外として ACS が定義する型・インターフェースへの参照を許可する（§4.6 参照） |
...
| 唯一の例外 | ACS が定義する型・インターフェースへの参照のみ許可する（§4.6 参照） |
| 相互依存 | 禁止 |

### §4.6 共有層定義

フレームワーク間で共有が必要な型定義・インターフェースは、ACS（Adlaire Client Services）エンジンに集約する。

| 規則 | 内容 |
|------|------|
| 共有層 | ACS エンジン全体が共有層を担う |
| 集約対象 | 通信契約型、リクエスト/レスポンス型、API エンドポイント契約、共通エラー型、共有インターフェース |
| 参照許可範囲 | ACS の全コンポーネント（Interface / Class / Core / Api / Utils）への参照を許可する |
| 参照方向 | 他エンジン → ACS の一方向のみ。ACS が他エンジンを参照してはならない |
| 集約禁止 | ビジネスロジック固有の型、特定エンジン内部でのみ使用する型は各エンジン自身に定義する |
...
| 共有型の参照 | ACS の各コンポーネントからの import のみ、フレームワーク間で許可（§4.6 参照） |
