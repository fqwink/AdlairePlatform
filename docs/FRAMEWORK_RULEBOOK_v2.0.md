...
| フレームワーク間依存 | 依存ゼロ。フレームワーク間の直接 import / 参照を禁止（ACS への参照のみ例外） |
| 外部依存 | 外部ライブラリへの依存ゼロ |
...
| フレームワーク間依存ゼロ | フレームワーク間の直接 import / 参照を禁止。唯一の例外として ACS への参照のみ許可する（§4.6 参照） |
...
| **エンジン内コンポーネント間協調** | 同一エンジン内の 5 コンポーネントは相互参照が可能。他エンジンへの参照は禁止（ACS への参照のみ例外） |
| **外部依存ゼロ** | サードパーティライブラリ・外部パッケージへの依存を禁止する。言語標準ライブラリのみ使用可 |
| **エンジン単位の自己完結** | 5 ファイル一体で自己完結する。他エンジン（フレームワーク）の存在を前提としない |
| **フレームワーク間依存ゼロ** | エンジン間の直接 import / 参照を禁止。唯一の例外として ACS への参照のみ許可する（§4.6 参照） |
