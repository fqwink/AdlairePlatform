...
| 特徴 | 説明 |
|------|------|
| **5 エンジン固定構成** | 各フレームワークは Core / Api / Utils / Interface / Class の 5 エンジンで構成される。エンジンの追加・省略は例外規定（§4.4）に従う |
| **1 ファイル = 1 エンジン** | 各エンジンは単一ファイルに格納される。1 ファイル内に複数クラスの配置を許容する |
| **責務の固定割当** | 各エンジンの責務は固定されており、エンジン間で責務を入れ替えてはならない |
| **エンジン間の暗黙的協調** | 同一フレームワーク内のエンジンは相互参照が可能。ただしフレームワーク外への参照は禁止 |
| **外部依存ゼロ** | サードパーティライブラリ・外部パッケージへの依存を禁止する。言語標準ライブラリのみ使用可 |
| **自己完結型** | 各フレームワークは単体でロード・実行可能。他フレームワークの存在を前提としない |
| **インターフェース契約** | フレームワーク間の通信は Interface エンジンが定義する契約に基づく。具象クラスへの直接依存を禁止 |
