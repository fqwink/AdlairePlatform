...
| **モジュラーコンポーネント** | 各ファイルはエンジンの構成部品（モジュラーコンポーネント）である。1 コンポーネント内に複数クラスの配置を許容する |
| **責務の固定割当** | 5 コンポーネントそれぞれの責務は固定されており、コンポーネント間で責務を入れ替えてはならない |
...
各モジュラーコンポーネントの責務は以下の通り固定する。責務外のコードを配置してはならない。

| コンポーネント | 責務 | 配置するもの | 配置禁止 |
|--------------|------|------------|---------|
| **Core** | ビジネスロジック | サービスクラス、ドメインロジック、状態管理 | ルーティング、データ型定義 |
| **Api** | 外部との入出力境界層 | エンジンと外部世界の境界処理（詳細は §4.2.1） | ビジネスロジック、データ型定義 |
...
1 エンジン = **5 モジュラーコンポーネント**で構成する。各コンポーネントはエンジンの構成部品であり、単独では機能しない。

| コンポーネント | 役割 |
|--------------|------|
| **Core** | ビジネスロジック |
| **Api** | 外部との入出力境界層（§4.2.1 参照） |
| **Utils** | ユーティリティ・補助機能 |
