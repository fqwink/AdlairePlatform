...
### 3.3 コレクションエンドポイント（認証不要・Ver.1.4 追加）
...
### 3.4 管理エンドポイント（API キーまたはセッション認証）
...
> **Ver.1.4-pre 更新**: メール送信は直接 `mail()` を呼び出さず、`MailerEngine::sendContact()` を使用するように変更されました。
> MailerEngine はリトライ機能、ヘッダインジェクション防止、送信ログ記録を内部で処理します。
...
private static function sendContact(): array {
    ...
    // メール送信（MailerEngine 経由）
    // MailerEngine がリトライ・ヘッダインジェクション防止・ログ記録を処理
    $result = MailerEngine::sendContact($name, $email, $message);
...
| 事項 | ステータス | 内容 |
|------|-----------|------|
| 管理エンドポイントの認証方式 | ✅ 確定 | API キー（Bearer トークン）+ セッション認証のハイブリッド方式を採用。`api_keys.json` に bcrypt ハッシュ保存 |
| `contact` のメール送信設定 | ✅ 確定 | `MailerEngine::sendContact()` を使用（Ver.1.4-pre）。リトライ・ヘッダインジェクション防止・ログ記録を内蔵。宛先アドレスは `settings.json` の `contact_email` キーに保存 |
