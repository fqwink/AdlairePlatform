...
> **分類**: 社内限り
>
> **✅ 実装完了**: StaticEngine は Ver.1.3-28 で実装されました。
> 本設計書（Ver.0.3-2）は実装済みの仕様を記録しています。
>
> **Ver.1.4-pre 互換**: Ver.1.4-pre で導入された JsonCache により、ビルド時の JSON 読み込みがキャッシュされビルドパフォーマンスが向上します。
> また、Logger を使用してビルド進捗のログ出力が可能になりました。
...
| テーマエンジン流用 | `TemplateEngine::render()` + `ThemeEngine::buildStaticContext()` でレンダリング | ✅ 確定 |
...
`TemplateEngine` + `ThemeEngine::buildStaticContext()` により、テンプレートに渡すコンテキストを切り替えるだけで管理者 UI を除外します。`ob_start()` / `stripAdminUI()` は不要です。
...
`stripAdminUI()` による正規表現の事後操作が不要になり、テーマ構造に依存しない安全なレンダリングが実現します。

### 7.2 アセットパスの書き換え

静的 HTML 内のアセットパスは `renderPage()` 内で文字列置換します:
...
| Step 2 | `StaticEngine::renderPage()` — `TemplateEngine::render()` + `ThemeEngine::buildStaticContext()` でレンダリング | 高 |
...
| バージョン | 日付 | 変更内容 |
|------------|------|---------|
| Ver.0.3-1 | 2026-03-08 | TemplateEngine 導入に伴うレンダリング戦略の刷新。`ob_start()` + `stripAdminUI()` 方式を廃止し、`TemplateEngine::render()` + `ThemeEngine::buildStaticContext()` 方式に変更。テーマは `theme.html`（PHP フリー）を優先 |
