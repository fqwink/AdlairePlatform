...
> **Ver.1.3-27**: 本ドキュメントは Ver.1.3-27 時点のアーキテクチャを記録しています。
> AdminEngine 導入・管理ダッシュボード化が完了。StaticEngine・ApiEngine は Ver.1.3系で引き続き実装予定。
...
| ユーティリティ | `h()` | HTMLエスケープ |
| レガシーラッパー | `is_loggedin()`, `csrf_token()`, `verify_csrf()`, `content()`, `editTags()`, `menu()` | AdminEngine に委譲（theme.php フォールバック互換用） |
...
### engines/StaticEngine.php（計画 — Ver.1.3系で実装予定）
