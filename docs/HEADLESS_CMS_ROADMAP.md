...
> **ドキュメントバージョン**: Ver.1.3-draft
> **ステータス**: 🔧 デバッグ・バグ修正フェーズ（Ver.1.3 リリース準備中）
> **作成日**: 2026-03-08
> **最終更新**: 2026-03-09
> **所有者**: Adlaire Group
> **分類**: 社内限り
...
| 機能 | pitcms | AP 現状（Ver.1.3-28） | ギャップ |
|------|--------|----------------------|---------|
...
### Phase 0: 基盤整備（現在 → 次期リリース）

> 既存機能の安定化。Plan ファイルの提案 A/B/C を優先実装。
...
### Phase 1: Markdown 対応 + コレクション基盤

> コンテンツを Markdown で記述できるようにし、コレクション構造を導入。
...
> コンテンツを GitHub リポジトリと同期。pitcms の核心機能。
...
### Phase 3: プレビュー + レビューワークフロー

> pitcms のプレビュー環境・レビュー承認フローを実現。
...
> **Ver.1.3 系ステータス**: デバッグ・バグ修正実施後、正式リリース予定
