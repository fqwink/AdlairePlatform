...
AdlairePlatform Ver.1.3-27 の実装済み機能をソースコード解析に基づいて整理したドキュメントです。

> 最終更新: 2026-03-08
> **Ver.1.3系開発中**: AdminEngine 導入・管理ダッシュボード化を完了（Ver.1.3-27）。
...
```
themes/<テーマ名>/
├── theme.html      # テンプレートエンジン方式（推奨・PHP フリー）
├── theme.php       # レガシー PHP 方式（フォールバック）
└── style.css       # スタイルシート
```

> **注**: `settings.html` は Ver.1.3-27 でダッシュボードに統合され、テーマから削除されました。
...
| `content(string $id, string $content)` | `void` | AdminEngine::renderEditableContent() へ委譲（レガシーラッパー） |
| `editTags()` | `void` | AdminEngine のフック出力へ委譲（レガシーラッパー） |
...
| モジュール | 設計書 | ステータス | 説明 |
|----------|--------|-----------|------|
| AdminEngine・ダッシュボード | — | ✅ 実装済み（Ver.1.3-27） | 管理エンジン導入・ダッシュボード UI（`engines/AdminEngine.php` / `?admin`） |
| StaticEngine | `docs/STATIC_GENERATOR.md` Ver.0.3-1 | 🔜 実装予定 | 静的サイト生成エンジン（`engines/StaticEngine.php`） |
| ApiEngine | `docs/HEADLESS_CMS.md` Ver.0.3-1 | 🔜 実装予定 | ヘッドレス CMS REST API エンジン（`engines/ApiEngine.php`） |
...
*このドキュメントは `index.php`・`engines/` 配下のファイル（AdminEngine.php 含む）・`themes/*/theme.php`・`.htaccess` のソースコード解析に基づいて生成されました。*
