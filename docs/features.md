...
| `handle_update_action()` | 285 | `void` | POST `ap_action` をディスパッチ（認証・CSRF 検証済み）、JSON を返して終了 |
| `check_update()` | 333 | `array` | GitHub Releases API から最新バージョン情報を取得・AP_VERSION と比較 |
| `backup_current()` | 365 | `string` | `backup/YYYYMMDD_His/` に全ファイルをコピー（`data/`・`backup/`・`.git/` 除外） |
| `apply_update(string $zip_url, string $new_version)` | 388 | `void` | ZIP ダウンロード → 展開 → `data/`・`backup/` 保護して上書き → `version.json` 更新 |
| `rollback_to_backup(string $backup_name)` | 462 | `void` | 指定バックアップから `data/` を除いてファイルを復元 |
| `csrf_token()` | 487 | `string` | セッション CSRF トークンを生成・取得 |
| `verify_csrf()` | 494 | `void` | POST / ヘッダーの CSRF トークンを検証（失敗時 403 終了） |
| `host()` | 501 | `void` | リクエスト URI からホスト URL・ページスラッグを解析 |
| `settings()` | 514 | `void` | テーマ選択・各種設定フォーム・アップデートパネルを出力 |
...
| 自動バックアップ | 更新適用前に `backup/YYYYMMDD_His/` へ全ファイルをコピー |
...
| パストラバーサル | `backup_name` を `^[0-9_]+$` でバリデーション |
