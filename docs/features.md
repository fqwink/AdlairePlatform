...
AdlairePlatform Ver.1.2-26（Ver.1.2系最終リビジョン）の実装済み機能をソースコード解析に基づいて整理したドキュメントです。

> 最終更新: 2026-03-08
> **⚠️ Ver.1.2系は Ver.1.2-26 をもって終了しました。** 以降の機能追加は次期メジャーバージョン（Ver.2.x）にて実施予定です。
...
## Ver.1.3系で実装予定（設計確定済み）

> Ver.1.3系にて以下の機能を実装予定です。

| モジュール | 設計書 | ステータス | 説明 |
|----------|--------|-----------|------|
| StaticEngine | `docs/STATIC_GENERATOR.md` Ver.0.3-1 | 🔜 Ver.1.3系で実装予定 | 静的サイト生成エンジン（`engines/StaticEngine.php`） |
| ApiEngine | `docs/HEADLESS_CMS.md` Ver.0.3-1 | 🔜 Ver.1.3系で実装予定 | ヘッドレス CMS REST API エンジン（`engines/ApiEngine.php`） |
| ダッシュボード化 | — | 🔜 Ver.1.3系で実装予定 | 管理ツールをダッシュボード形式に刷新 |
| 管理ツールエンジン化 | — | 🔜 Ver.1.3系で実装予定 | 管理ツールをエンジン駆動モデルに再設計 |
