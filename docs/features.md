...
## 未実装（設計確定 — 次期バージョン以降で実装予定）

> Ver.1.2系終了に伴い、以下の機能は次期メジャーバージョン（Ver.2.x）以降で実装予定です。

| モジュール | 設計書 | ステータス | 説明 |
|----------|--------|-----------|------|
| StaticEngine | `docs/STATIC_GENERATOR.md` Ver.0.3-1 | ✅ 設計確定（Ver.1.2系では未実装） | 静的サイト生成エンジン（`engines/StaticEngine.php`） |
| ApiEngine | `docs/HEADLESS_CMS.md` Ver.0.3-1 | ✅ 設計確定（Ver.1.2系では未実装） | ヘッドレス CMS REST API エンジン（`engines/ApiEngine.php`） |
