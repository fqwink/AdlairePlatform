...
AdlairePlatform Ver.1.5.0 の実装済み機能・未実装機能・関数リファレンス・API リファレンスを定めたドキュメントです。

> 最終更新: 2026-03-14
> **Ver.1.5.0**: 全 16 エンジン + 4 補助を Framework（APF/ACE/AIS/ASG/AEB/ADS）に統合完了。Ver.1.3 系は非推奨。
...
### DiagnosticEngine（engines/DiagnosticEngine.php）⭐ Ver.1.4-pre
...
Ver.1.5 で導入された Framework 統合基盤。engines/ の static ファサードを維持しつつ、内部を Framework モジュールに委譲。

### インフラ基盤

| 機能 | 説明 | 実装場所 |
|------|------|---------|
| 名前空間オートローダー | `spl_autoload_register` で Framework/ の 12 名前空間プレフィックスを自動解決 | `autoload.php` |
...
| ユーティリティ集約 | index.php から分離した全グローバル関数（JsonCache, data_dir, json_read 等） | `engines/Bridge.php` |

### Static Facade パターン（全 16 エンジン）

| 機能 | 説明 | 実装場所 |
|------|------|---------|
| Framework インスタンス getter | 各エンジンに lazy-init の Framework クラスインスタンス取得メソッドを追加 | 全 `engines/*.php` |
| 後方互換維持 | 既存の static メソッド API は一切変更なし | 全 `engines/*.php` |
...
| AEB エディタアダプター | ES6 モジュールをグローバルスコープ（`window.AEB`）に公開 | `engines/JsEngine/aeb-adapter.js` |
