...
`engines/JsEngine/wysiwyg.js` による独自実装（依存ライブラリなし・ES5 互換）。

### ツールバー機能

| ボタン | 機能 | execCommand / 実装 |
|--------|------|-------------------|
| B | 太字 | `bold` |
| I | 斜体 | `italic` |
| U | 下線 | `underline` |
| 🔗 | リンク挿入 | `createLink` |
| H2 / H3 | 見出し変換 | `formatBlock` |
| • | 箇条書き | `insertUnorderedList` |
| 1. | 番号リスト | `insertOrderedList` |
| ❝ | 引用ブロック | `formatBlock blockquote` |
| {} | コードブロック | `formatBlock pre` |
| — | 区切り線 | `insertHTML <hr>` |
| ⊞ | テーブル挿入 | `insertHTML` + グリッドピッカー |
| ↩ / ↪ | Undo / Redo | `undo` / `redo` |
| ✕ | 書式クリア | `removeFormat` |
| 🖼 | 画像挿入 | `upload_image()` API |

### 画像機能

| 機能 | 説明 |
|------|------|
| 挿入方法 | D&D / クリップボード貼付 / ボタン選択 の3通り |
| 許可形式 | JPEG / PNG / GIF / WebP |
| リサイズ | 4コーナーハンドル・アスペクト比維持 |
| alt 編集 | リサイズ枠の下部に alt 属性入力欄（即時反映） |
...
|------|------|
| 挿入 | 8×8 グリッドピッカーで挿入 |
| 行操作 | テーブル内カーソル時に行追加・削除バーを表示 |
| 列操作 | テーブル内カーソル時に列追加・削除バーを表示 |
...
| 機能 | 説明 |
|------|------|
| 起動 | 空行で `/` を入力するとブロックタイプ選択メニューが表示される |
| 絞り込み | `/コ` のようにタイプすることでインクリメンタル絞り込みが可能 |
| キーボード操作 | ArrowDown/Up で移動・Enter で確定・Escape で閉じる |
| 対象ブロック | 段落・H2・H3・引用・コード・箇条書き・番号リスト・区切り線・テーブル・画像挿入 |

### Ph3: ブロックハンドル・タイプ変換

| 機能 | 説明 |
|------|------|
| ハンドル表示 | ブロックにホバーすると左端へ `⠿` ハンドルを表示（300ms 遅延タイマー） |
