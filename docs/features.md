...
16. [Framework 統合 (Ver.1.5)](#16-framework-統合-ver15)
...
| インプレイス編集（plain text） | ログイン中はコンテンツ領域をクリックしてその場で編集・Fetch API 保存 | `Framework/AP/JsEngine/editInplace.js` |
| インプレイス編集（WYSIWYG） | `class="editRich"` 要素クリックで WYSIWYG エディタを起動 | `Framework/AP/JsEngine/wysiwyg.js` |
| マルチページ管理 | スラッグベースの URL でページを識別・管理 | `index.php` |
| 設定管理 | タイトル・説明・キーワード・著作権・メニュー・テーマ選択をダッシュボードから編集 | `Framework/ACE/ACE.Admin.php` |
| サイドコンテンツ | サイドバー用の独立したコンテンツ領域（`subside`）を管理 | `index.php` |
| 画像アップロード | 認証済みユーザーが JPEG/PNG/GIF/WebP 画像をアップロード（最大 2MB） | `Framework/ACE/ACE.Admin.php: handleUploadImage()` |
...
| 機能 | 説明 | 実装場所 |
|------|------|---------|
| テーマ切替 | ダッシュボードのドロップダウンからリアルタイムでテーマを切替 | `Framework/ACE/ACE.Admin.php`, `Framework/AP/JsEngine/dashboard.js` |
| テーマ自動検出 | `themes/` ディレクトリを走査して利用可能なテーマを自動列挙 | `Framework/ACE/ACE.Core.php: listThemes()` |
| テーマバリデーション | テーマ名を正規表現 `[a-zA-Z0-9_-]` でバリデーション（パストラバーサル防止） | `Framework/ACE/ACE.Core.php: load()` |
| デフォルトフォールバック | 指定テーマが存在しない場合 `AP-Default` にフォールバック | `Framework/ACE/ACE.Core.php: load()` |
| ネストプロパティ | `{{user.name}}` → `$ctx['user']['name']` のドット記法アクセス | `Framework/ACE/ACE.Core.php` |
| フィルター | `upper`, `lower`, `capitalize`, `trim`, `nl2br`, `length`, `truncate:N`, `default:value` | `Framework/ACE/ACE.Core.php` |
| フィルターチェーン | `{{var\|trim\|upper}}` のようにフィルターをパイプで連結 | `Framework/ACE/ACE.Core.php` |
...
`Framework/AP/JsEngine/wysiwyg.js` による独自実装（依存ライブラリなし・ES2020+）。
...
| 機能 | 説明 | 実装場所 |
|------|------|---------|
| シングルパスワード認証 | `auth.json` に保存された bcrypt ハッシュと照合 | `Framework/ACE/ACE.Admin.php: login()` |
| パスワード変更 | ログイン済み状態でパスワードを変更・即時 bcrypt 保存 | `Framework/ACE/ACE.Admin.php: savePassword()` |
| MD5 → bcrypt 自動移行 | 旧 MD5 ハッシュを検出した場合に管理者へ警告を表示 | `Framework/ACE/ACE.Admin.php: login()` |
| セッション認証 | `$_SESSION['l']` によるログイン状態管理 | `Framework/ACE/ACE.Admin.php: isLoggedIn()` |
| レート制限 | 5回失敗で15分ロックアウト（IP ベース・`login_attempts.json`） | `Framework/ACE/ACE.Admin.php: checkLoginRate()` |

### セキュリティ対策

| 対策 | 説明 | 実装場所 |
|------|------|---------|
| bcrypt ハッシュ化 | `password_hash($p, PASSWORD_BCRYPT)` によるパスワード保護 | `Framework/ACE/ACE.Admin.php: savePassword()` |
| CSRF トークン | `random_bytes(32)` 生成・`empty()` + `hash_equals()` で検証 | `Framework/ACE/ACE.Admin.php: verifyCsrf()` |
| XSS エスケープ | `h()` 関数（`htmlspecialchars(ENT_QUOTES, UTF-8)`）を全出力に適用 | `index.php: h()` |
| セッション固定攻撃対策 | ログイン成功時に `session_regenerate_id(true)` を実行 | `Framework/ACE/ACE.Admin.php: login()` |
| セキュアクッキー | `HttpOnly`・`SameSite=Lax` をセッションクッキーに設定 | `index.php` |
| パストラバーサル防止 | テーマ名・フィールド名・バックアップ名を正規表現でバリデーション | `Framework/ACE/ACE.Admin.php`, `Framework/AIS/AIS.Deployment.php` |
| ディレクトリ保護 | `data/`・`files/`・`backup/`・`Framework/*.php` への直接 HTTP アクセスを拒否 | `.htaccess` |
| セキュリティヘッダー | CSP / `X-Content-Type-Options` / `X-Frame-Options` / `Referrer-Policy` を付与 | `.htaccess` |
| 画像アップロード保護 | MIME 検証（`finfo`）・2MB 制限・ランダムファイル名・PHP 実行不可 | `Framework/ACE/ACE.Admin.php: handleUploadImage()` |
| アップデート URL 検証 | `apply_update()` は GitHub ドメインの URL のみ受け入れ | `Framework/AIS/AIS.Deployment.php` |
| バックアップ名バリデーション | `^[0-9_]+$` のみ許可（二重防御） | `Framework/AIS/AIS.Deployment.php` |
| WYSIWYG サニタイザー | 保存前にホワイトリスト方式で不正タグを除去 | `Framework/AP/JsEngine/wysiwyg.js` |
...
| 機能 | 説明 | 実装場所 |
|------|------|---------|
| Fetch API インライン保存 | コンテンツ編集を画面遷移なしに保存 | `Framework/AP/JsEngine/editInplace.js` |
| テキストエリア自動拡張 | autosize により入力量に応じてテキストエリアが自動伸長 | `Framework/AP/JsEngine/autosize.js` |
| `<br>` ↔ 改行 変換 | 保存時に改行 → `<br>`、編集時に `<br>` → 改行 を変換 | `Framework/AP/JsEngine/editInplace.js` |
| テーマ即時反映 | テーマ選択変更後にページをリロードして CSS を即時適用 | `Framework/AP/JsEngine/dashboard.js` |
| 管理ダッシュボード | `?admin` でテーマ非依存の管理画面を表示 | `Framework/ACE/ACE.Admin.php` |
| WYSIWYGエディタ | 依存ライブラリなしの完全独自実装 | `Framework/AP/JsEngine/wysiwyg.js` |
| アップデート UI | 更新確認・適用・ロールバック操作を管理パネルから実行 | `Framework/AP/JsEngine/updater.js` |
...
| `Framework/*.php` | 直接アクセスを 403 Forbidden で拒否 |
...
`Framework/ACE/ACE.Admin.php` に定義された主要メソッド:
...
`Framework/AIS/AIS.Deployment.php` に定義された主要関数:
...
管理画面からワンクリックで CMS 本体を更新できるエンジン（`Framework/AIS/AIS.Deployment.php`）。
...
`Framework/ASG/ASG.Core.php` による静的サイト生成エンジン。
...
| 管理 UI | 差分/フルビルド・クリーン・ZIP ボタン | `Framework/AP/JsEngine/static_builder.js` |

---

## 10. ヘッドレス CMS / API

`Framework/ACE/ACE.Api.php` による公開 REST API。
...
| レスポンスキャッシュ | ファイルベースキャッシュ | `Framework/AIS/AIS.System.php` |
| API クライアント | 静的サイト向け軽量クライアント | `Framework/AP/JsEngine/ap-api-client.js` |
| API キー管理 UI | 生成・削除 | `Framework/AP/JsEngine/api_keys.js` |

---

## 11. コレクション管理

`Framework/ACE/ACE.Core.php` + `Framework/ACE/ACE.Core.php` によるコレクション管理。
...
| 管理 UI | コレクション・アイテムの CRUD 操作 | `Framework/AP/JsEngine/collection_manager.js` |

---

## 12. Git 連携

`Framework/AIS/AIS.Deployment.php` による GitHub リポジトリ連携。
...
| 管理 UI | Pull/Push 操作 | `Framework/AP/JsEngine/git_manager.js` |

---

## 13. Webhook

`Framework/ACE/ACE.Api.php` による Webhook 管理・送信。
...
| 管理 UI | 登録・削除・テスト送信 | `Framework/AP/JsEngine/webhook_manager.js` |

---

## 14. 画像最適化

`Framework/AIS/AIS.Deployment.php` による画像最適化。
...
### AppContext（Framework/AIS/AIS.Core.php）
...
### Logger（Framework/APF/APF.Utilities.php）
...
### MailerEngine（Framework/AIS/AIS.Core.php）
...
### DiagnosticEngine（Framework/AIS/AIS.System.php）⭐ Ver.1.4-pre

リアルタイム診断・テレメトリエンジン。

| 機能 | 実装 |
|------|------|
| PHP エラー/例外/Fatal の自動キャプチャ | ✅ |
| 12カテゴリ分類（runtime, security, integration 等） | ✅ |
| カスタムログ記録 API（`DiagnosticEngine::log()`） | ✅ |
| パフォーマンスプロファイラ（エンジン別タイマー） | ✅ |
| セキュリティイベント集計（ログイン失敗・ロックアウト・SSRF） | ✅ |
| リアルタイム送信（未送信分を毎リクエスト POST） | ✅ |
| サーキットブレーカー（連続送信失敗時の自動停止） | ✅ |
| ログローテーション（3世代） | ✅ |
| 14日間保持ポリシー | ✅ |
| PII マスク・センシティブキー除外 | ✅ |
| ダッシュボード UI（ヘルスチェック・トレンド・操作） | ✅ |

---

## 16. Framework 統合 (Ver.1.5)

Ver.1.5 で導入された Framework 統合基盤。engines/ の static ファサードは完全に削除され、Framework モジュールを直接使用する構成に移行済み。

### インフラ基盤

| 機能 | 説明 | 実装場所 |
|------|------|---------|
| 名前空間オートローダー | `spl_autoload_register` で Framework/ の 13 名前空間プレフィックスを自動解決（`AP\Controllers\` 含む） | `autoload.php` |
| DI コンテナ | `APF\Core\Container` を使った依存性注入 | `bootstrap.php` |
| Application ファサード | Container, HookManager, EventDispatcher の統合アクセス | `bootstrap.php` |
| ユーティリティ集約 | index.php から分離した全グローバル関数（JsonCache, data_dir, json_read 等） | `Framework/AP/AP.Bridge.php` |

### フロントエンド統合

| 機能 | 説明 | 実装場所 |
|------|------|---------|
| ADS デザインシステム | CSS 変数・コンポーネント・エディタスタイルを dashboard + テーマに注入 | `dashboard.html`, `AdminEngine::registerHooks()` |
| AEB エディタアダプター | ES6 モジュールをグローバルスコープ（`window.AEB`）に公開 | `Framework/AP/JsEngine/aeb-adapter.js` |

### Framework モジュール一覧

| モジュール | エンジン数 | 内容 |
|----------|-----------|------|
| APF (Adlaire Platform Foundation) | 3 | Container, Router, Request, Response, Validator, Cache, Logger |
| ACE (Adlaire Content Engine) | 3 | CollectionManager, AuthManager, ApiRouter, WebhookManager |
| AIS (Adlaire Infrastructure Services) | 3 | AppContext, CacheStore, AppLogger, DiagnosticsCollector, Updater, GitSync, Mailer |
| ASG (Adlaire Static Generator) | 3 | Generator, TemplateRenderer, MarkdownParser, ImageOptimizer |
| AEB (Adlaire Editor & Blocks) | 3 | Editor, EventBus, BlockRegistry, 10 ブロックタイプ, sanitizer |
| ADS (Adlaire Design System) | 3 | Base CSS 変数, Components, Editor スタイル |
...
> 保留事項・次期バージョン検討項目の詳細は [AdlairePlatform_Design.md §7 バージョン計画 — 保留事項](AdlairePlatform_Design.md) を参照してください。

---

*このドキュメントは `index.php`・`Framework/` 配下の全モジュール・`.htaccess` のソースコード解析に基づいて生成されました。*
