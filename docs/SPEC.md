...
8. [静的サイト生成エンジン](#8-静的サイト生成エンジン)
9. [ヘッドレス CMS / API エンジン](#9-ヘッドレス-cms--api-エンジン)
...
```
Ver.1.2系（終了）                 Ver.1.3系（Ver.1.3-28 現在）
──────────────────────────     ───────────────────────────────
フラットファイル CMS（確定）  →    継続
...
WYSIWYGエディタ（完了）      →    ─
─                            →    AdminEngine・ダッシュボード化（✅ Ver.1.3-27）
─                            →    ApiEngine 実装（✅ Ver.1.3-28）
─                            →    CollectionEngine 実装（✅ Ver.1.3-28）
─                            →    MarkdownEngine 実装（✅ Ver.1.3-28）
─                            →    GitEngine 実装（✅ Ver.1.3-28）
─                            →    WebhookEngine 実装（✅ Ver.1.3-28）
─                            →    CacheEngine 実装（✅ Ver.1.3-28）
─                            →    ImageOptimizer 実装（✅ Ver.1.3-28）
```

> **注**: Ver.1.2系は Ver.1.2-26 をもって終了。Ver.1.3系で全エンジンの実装を完了。
...
│   ├── features.md               # 実装機能一覧
│   ├── VERSIONING.md
│   ├── STATIC_GENERATOR.md       # StaticEngine 設計書
│   ├── HEADLESS_CMS.md           # ApiEngine 設計書
│   ├── HEADLESS_CMS_ROADMAP.md   # ヘッドレス CMS ロードマップ
│   └── Licenses/
