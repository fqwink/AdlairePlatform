...
> **ドキュメントバージョン**: Ver.0.2-9
> **ステータス**: ✅ 確定（Ver.1.3-27 / AdminEngine・ダッシュボード反映）
> **作成日**: 2026-03-06
> **最終更新**: 2026-03-08（AdminEngine・ダッシュボード化を反映）
> **所有者**: Adlaire Group
> **バージョニング規則**: [AFE/VERSIONING.md](https://github.com/fqwink/AdlaireGroup-Documents-Repository/blob/main/AFE/VERSIONING.md)
>
> **Ver.1.3系開発中**: AdminEngine 導入・管理ダッシュボード化を完了（Ver.1.3-27）。StaticEngine・ApiEngine は引き続き実装予定。
...
```
Ver.1.2系（終了）                 Ver.1.3系
──────────────────────────     ───────────────────────────────
フラットファイル CMS（確定）  →    継続
動的ページ配信（確定）       →    StaticEngine 実装（設計確定済み）
jQuery 全廃（完了）          →    ─
PHP 8.2+ 必須化（完了）      →    ─
TemplateEngine 導入（完了）  →    ─
WYSIWYGエディタ（完了）      →    ─
─                            →    ApiEngine 実装（設計確定済み）
─                            →    管理ツールのダッシュボード化（✅ Ver.1.3-27）
─                            →    管理ツールのエンジン駆動モデル化（✅ Ver.1.3-27）
```

> **注**: Ver.1.2系は Ver.1.2-26 をもって終了。Ver.1.3-27 で AdminEngine 導入・ダッシュボード化を完了。StaticEngine・ApiEngine は引き続き実装予定。
...
> **注**: Ver.1.2 まで存在していた `settings.html`（管理者設定パネル パーシャル）は Ver.1.3-27 でダッシュボードに統合され、廃止されました。
...
- D&D / クリップボード貼付 / ボタン選択 / スラッシュコマンドの4通りで挿入
...
- チェックボックスクリックで状態切替
- Enter で新項目追加、空項目で Backspace で削除

#### 5.5.9 スラッシュコマンドメニュー

- 空ブロックで `/` を入力するとブロックタイプ選択メニューが表示される
- インクリメンタル絞り込み対応
- ArrowDown/Up で選択移動・Enter で確定・Escape で閉じる
- ビューポートクランプ（画面端での自動位置調整）

#### 5.5.10 ブロックハンドル・タイプ変換

- ブロックにホバーすると左端へ `⠿` ハンドルを表示
- ハンドルクリックでポップアップメニュー表示
- ブロックタイプ変換（互換性のあるタイプ間で内容保持）
...
- ホワイトリスト方式 HTML サニタイザー（保存前に不正タグ除去）
...
| セクション | 内容 |
|-----------|------|
| サイト設定 | テーマ選択・メニュー・タイトル・説明・キーワード・著作権の編集 |
| ページ一覧 | 全ページをグリッド表示、クリックで編集ページへ移動 |
| アップデート | バージョン表示・更新確認ボタン・結果表示 |
...
## 8. 静的生成ジェネレーター機能（Ver.1.3系で実装予定）

> ✅ **ステータス: 設計確定（Ver.1.3系で実装予定）**
>
> StaticEngine の設計は `docs/STATIC_GENERATOR.md`（Ver.0.3-1）にて承認済みです。
> Ver.1.3系にて実装を予定しています。
> 詳細は `docs/STATIC_GENERATOR.md` を参照してください。

---

## 9. ヘッドレス CMS 機能（Ver.1.3系で実装予定）
