...
> **ドキュメントバージョン**: Ver.0.2-7
> **ステータス**: ✅ 確定（Ver.1.2系終了）
> **作成日**: 2026-03-06
> **最終更新**: 2026-03-08（Ver.1.2-26 / Ver.1.2系終了）
> **所有者**: Adlaire Group
> **バージョニング規則**: [AFE/VERSIONING.md](https://github.com/fqwink/AdlaireGroup-Documents-Repository/blob/main/AFE/VERSIONING.md)
>
> **⚠️ Ver.1.2系は Ver.1.2-26 をもって終了しました。** 以降の機能追加・アーキテクチャ変更は次期メジャーバージョン（Ver.2.x）にて実施予定です。
...
```
Ver.1.2系（終了）                 次期 (v2.x 以降)
──────────────────────────     ───────────────────────────────
フラットファイル CMS（確定）  →    アーキテクチャ再設計（未検討）
...
> **注**: Ver.1.2系は Ver.1.2-26 をもって終了。StaticEngine・ApiEngine は設計確定済みですが、実装は次期バージョン以降で行います。
...
## 8. 静的生成ジェネレーター機能（次期バージョン以降で実装予定）

> ✅ **ステータス: 設計確定（Ver.1.2系では未実装 — 次期バージョン以降で実装予定）**
>
> StaticEngine の設計は `docs/STATIC_GENERATOR.md`（Ver.0.3-1）にて承認済みです。
> Ver.1.2系終了に伴い、実装は次期メジャーバージョン（Ver.2.x）以降に持ち越します。
> 詳細は `docs/STATIC_GENERATOR.md` を参照してください。

---

## 9. ヘッドレス CMS 機能（次期バージョン以降で実装予定）
