...
5. [§5 エンジンライフサイクル](#5-エンジンライフサイクル)
6. [§6 依存性注入・イベント駆動規則](#6-依存性注入イベント駆動規則)
7. [§7 命名規則](#7-命名規則)
8. [§8 フレームワーク登録簿](#8-フレームワーク登録簿)
9. [§9 ファイル操作制限・削除禁止事項](#9-ファイル操作制限削除禁止事項)
10. [§10 API フレームワーク規則](#10-api-フレームワーク規則)
11. [§11 オートローダー規則](#11-オートローダー規則)
12. [§12 フレームワーク新設・廃止手順](#12-フレームワーク新設廃止手順)
13. [§13 フレームワーク改良ロードマップ](#13-フレームワーク改良ロードマップ)
...
| ファイル構成 | **3〜7 ファイル構成**に集約化（基本: Core / Components / Utils） | ✅ 確定 |
...
各フレームワークは **最小 3 ファイル〜最大 7 ファイル**に集約化します。

| 規則 | 内容 | ステータス |
|------|------|-----------|
| 最小構成 | 3 ファイル | ✅ 確定 |
| 最大構成 | 7 ファイル | ✅ 確定 |
| 基本構成 | 3 ファイル（Core / Components / Utils） | ✅ 確定 |
| 8 ファイル以上 | **禁止** — 分割が必要な場合は新規フレームワーク化を検討する | ❌ 禁止 |
...
機能の明確な分離が必要な場合、**7 ファイルを上限**として追加エンジンファイルを配置できます。

| 条件 | 許容内容 | 承認 |
|------|---------|------|
| 4〜7 ファイルへの拡張 | 明確な責務分離理由がある場合に許容 | Adlaire Group 承認必須 |
...
すべてのエンジンは以下の **5 段階**のライフサイクルに従います。
...
| 段階 | タイミング | 実行回数 | 目的 |
|------|----------|---------|------|
| **登録** | アプリケーション起動前 | 1 回 | エンジンをフレームワークに認識させる |
| **初期化** | `boot()` 呼び出し時、依存順に実行 | 1 回 | 設定読み込み・リソース準備 |
| **起動** | 全エンジンの初期化完了後 | 1 回 | 他エンジンとの連携確立・イベントリスナー登録 |
| **実行** | 起動後〜シャットダウンまで | 多数回 | エンジン本来の機能提供 |
| **シャットダウン** | スクリプト終了時（自動）または明示呼び出し | 1 回 | リソース解放・クリーンアップ |
...
└─ AdminEngine::__construct()
    ↓
Initialize Phase（依存順・優先度順に実行）
├─ LoggerEngine::initialize()     [優先度: 10, 依存: なし]
├─ CacheEngine::initialize()      [優先度: 20, 依存: logger]
...
| 規則 | 内容 | ステータス |
|------|------|-----------|
| 初期化順序 | 依存グラフのトポロジカルソートに基づく自動解決 | ✅ 確定 |
| シャットダウン順序 | 初期化の逆順で実行 | ✅ 確定 |
| 循環依存 | 検出時にエラーとする（イベント駆動で回避すること） | ❌ 禁止 |
| ライフサイクルフック | `onInitialize()`, `onBoot()`, `onShutdown()` をオーバーライドして実装 | ✅ 確定 |
...
エンジン間の依存は **DI コンテナ経由**で解決します。

| 方式 | 用途 | メソッド |
|------|------|---------|
| `bind()` | 毎回新しいインスタンスを生成 | 状態を持たないサービス |
| `singleton()` | 初回取得時にインスタンス化、以降は同一インスタンス | DB接続、設定など |
| `instance()` | 既存インスタンスを直接登録 | 外部生成オブジェクトの登録 |

#### DI 規則

| 規則 | 内容 | ステータス |
|------|------|-----------|
| DI 経由の参照 | エンジンは他サービスを DI コンテナ経由で取得する | ✅ 確定 |
| 静的呼び出し禁止 | グローバルな静的メソッド呼び出しによる依存は禁止 | ❌ 禁止 |
| 依存宣言 | エンジンの `$dependencies` プロパティに依存先を明記する | ✅ 確定 |

### §6.2 イベント駆動アーキテクチャ

エンジン間の**疎結合な連携**にはイベント駆動を使用します。
...
| 形式 | 例 |
|------|-----|
| `{対象}.{動詞（過去形）}` | `user.created`, `data.updated`, `cache.cleared` |
| `{対象}.{動詞（現在進行形）}` | `user.creating`, `data.updating` |
| `{対象}.{名詞}.{動詞}` | `admin.session.started`, `api.request.received` |

#### イベント駆動規則

| 規則 | 内容 | ステータス |
|------|------|-----------|
| 疎結合優先 | エンジン間連携はイベント駆動を優先する | ✅ 確定 |
| 循環依存回避 | 循環依存が生じる場合はイベント駆動で解決する | ✅ 確定 |
| 優先度指定 | リスナーの実行順序は優先度（小さいほど先）で制御する | ✅ 確定 |
| 直接呼び出し抑制 | 他エンジンのメソッド直接呼び出しは最小限にとどめる | ✅ 確定 |

### §6.3 エンジン開発ベストプラクティス

| 原則 | 良い例 | 避けるべき例 |
|------|--------|------------|
| 単一責任 | 1 エンジン = 1 機能領域 | 複数の機能を1エンジンに詰め込む |
| イベント駆動 | `$this->emit('order.created', ...)` で通知 | 他エンジンのメソッドを直接呼び出す |
| 設定の外部化 | `getDefaultConfig()` でデフォルト設定を定義 | 設定値をハードコーディングする |
| 依存の明示 | `$dependencies` プロパティに依存先を列挙 | 暗黙的に他エンジンの存在を前提にする |
...
例:
  ...
  Adlaire Platform (Controllers) → AP（2文字例外: §7.1.1 参照）
```
...
| フレームワーク廃止時 | `$map` からエントリを削除する（§12 の手順に従う） |
...
7. **登録簿更新** — 本ルールブック §8 の登録簿を更新する（状態を「廃止」に変更）
8. **ドキュメント更新** — `Framework/README.md` および関連ドキュメントを更新する
9. **変更履歴記録** — `DOCUMENT_CHANGELOG.md` に記録する
...
| 方針 | 内容 | ステータス |
|------|------|-----------|
| AI/JIT 最適化 | 採用しない（複雑性回避、依存ゼロ原則維持） | ❌ 不採用 |
| PHP 標準機能活用 | OPcache, APCu 等の標準拡張で最適化 | ✅ 確定 |
| 外部ライブラリ | 導入しない（依存ゼロ原則） | ❌ 禁止 |
| 段階的改良 | フェーズ制で計画的に進行 | ✅ 確定 |

### §13.2 改良の4つの柱

| 柱 | 内容 |
|-----|------|
| 実践的パフォーマンス最適化 | PHP 標準機能で高速化（OPcache, 依存グラフキャッシング, 遅延ロード） |
| 堅牢性とセキュリティ | CSRF 強化, SQLi/XSS 対策, レート制限, セキュリティヘッダー |
| JavaScript フレームワーク化 | フロントエンド統一アーキテクチャ（ES6 モジュール化, イベント駆動統一） |
| 可観測性（Observability） | 構造化ロギング, イベントトレーシング, パフォーマンスプロファイラー |

### §13.3 フェーズ計画

| Phase | バージョン | 内容 | 期間 | 工数 | リリース目標 |
|-------|----------|------|------|------|------------|
| Phase 2 | Ver.2.2 | 実用的強化（構造化ロギング, イベントトレーシング, OPcache 最適化, DI 改善） | 3 週間 | 29h | 2026-04-04 |
| Phase 3 | Ver.2.3 | JavaScript フレームワーク化（APF-JS コア, サービス層, モジュール化） | 4 週間 | 48h | 2026-05-02 |
| Phase 4 | Ver.2.4 | 高度な最適化（APCu キャッシング, プリロード, 非同期処理, プロファイラー） | 2 週間 | 24h | 2026-05-16 |
| Phase 5 | Ver.2.5 | エンタープライズ対応（セキュリティ強化, 国際化 i18n, CLI ツール） | 2 週間 | 20h | 2026-05-30 |
| **合計** | — | — | **11 週間** | **121h** | — |

### §13.4 目標指標

| 指標 | 現状 (Ver.2.1) | 目標 (Ver.2.5) | 改善率 |
|------|---------------|---------------|--------|
| 起動時間 | 80ms | 15〜20ms | 75〜81% 削減 |
| メモリ使用量 | 7MB | 4MB | 43% 削減 |
| リクエスト処理 | 100 req/s | 400〜500 req/s | 4〜5 倍 |
| デバッグ時間 | 4h | 1h | 75% 削減 |
| JS 保守性 | 低 | 高 | フレームワーク化 |
