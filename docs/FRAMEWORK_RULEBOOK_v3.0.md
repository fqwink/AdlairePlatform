...
- **1 エンジン 5 ファイル原則**
  - 1 つのフレームワークは、1 つのエンジンとして構成しなければならない
  - 各エンジンは、必須の Core を含む最大 5 つのコンポーネントで構成される
  - Core のみが必須であり、それ以外のコンポーネントの有無と役割は各エンジンが自由に決定できる
- **モジュラーコンポーネント**
  - 各ファイルは、エンジンの独立した構成要素として機能する
  - 1 つのコンポーネント内には、複数のクラスを配置してよい
  - Core 以外のコンポーネントは、固定の役割名や責務に縛られず、エンジン全体の設計意図に沿う範囲で自由に構成できる
- **自己完結**
  - 各エンジンは、すべてのコンポーネントを含めて自己完結しなければならない。他のエンジンの存在を前提としてはならない

#### 依存規則

- **エンジン内協調** — 同一エンジン内のコンポーネント間で参照が必要な場合、必ず Core を介さなければならない。Core 以外のコンポーネント同士が直接参照することを禁止する
- **外部依存ゼロ** — サードパーティライブラリおよび外部パッケージへの依存を禁止する。使用できるのは言語標準ライブラリのみとする
- **フレームワーク間依存ゼロ** — ACS を含むすべてのフレームワーク間において、直接の `import` を一切禁止する。サーバとの通信は `globalThis.__acs` を経由しなければならない（§3 参照）
- **共有型ファイルの作成禁止** — `Framework/types.ts` 等の共有型ファイルを新規に作成してはならない。各フレームワークは、必要な型を自ら定義するか、もしくは ACS が公開する `ACS.d.ts` から `import type` により参照しなければならない

#### ACS アクセス規則

- ACS は `globalThis.__acs` 上にシングルトンとして公開される（§3 参照）
- 各フレームワークは ACS を直接 `import` せず、`globalThis.__acs` を通じて ACS の機能を利用しなければならない
- `globalThis.__acs` を参照できるのは、各エンジンの Core コンポーネントのみとする。Core 以外のコンポーネントが `globalThis.__acs` を直接参照してはならない
- 型の参照は `ACS/ACS.d.ts` からの `import type` のみを許可する。値を伴う `import` は禁止する
...
ACS（Adlaire Client Services）は、サーバ（ASS）との通信を一元的に担うクライアントサイドのエンジンである。各フレームワークは ACS の実装ファイルを直接 `import` することなく、`globalThis.__acs` を通じて ACS の機能を利用する。

### §3.2 責務一覧

ACS が担う責務は以下の 16 項目であり、カテゴリごとに分類して示す。
...
ACS はブートストラップ時に、以下の構造を `globalThis.__acs` として公開する。
...
- 各フレームワークは `ACS.d.ts` を `import type` でのみ参照できる
- 値を伴う `import`（実装の `import`）は禁止する
...
| ルール | 内容 |
|--------|------|
| **実装の import 禁止** | 各フレームワークは ACS の実装ファイルを `import` してはならない |
| **globalThis 経由** | サーバとの通信はすべて `globalThis.__acs` を経由しなければならない |
| **Core 限定参照** | `globalThis.__acs` を参照できるのは各エンジンの Core コンポーネントのみとする。Core 以外のコンポーネントが直接参照してはならない |
| **型参照のみ許可** | `ACS.d.ts` からの `import type` のみを許可する。値を伴う `import` は禁止する |
| **直接 fetch 禁止** | 各フレームワークは `fetch()` を直接呼び出してはならない。すべての HTTP 通信は ACS を経由しなければならない |
| **初期化順序** | ACS の初期化（`globalThis.__acs` の公開）は、ブートストラップ処理において最初に実行しなければならない |
