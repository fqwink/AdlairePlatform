...
- **1 エンジン 5 ファイル原則** — 1 フレームワーク = 1 エンジン。Core を含む最大 5 つのコンポーネントが一体となり動作する。Core のみ定義必須であり、他コンポーネントの有無・役割はエンジンごとに自由に決定できる
- **自己完結** — コンポーネント一体で自己完結する。他エンジンの存在を前提としない
- **モジュラーコンポーネント** — 各ファイルはエンジン一体型の構成部品。1 コンポーネント内に複数クラスの配置を許容する。Core 以外のコンポーネントは固定の役割名・責務を定義せず、エンジン全体の設計意図に沿う範囲で自由に構成できる
...
- **ACS グローバルシングルトン** — ACS は `globalThis.__acs` にシングルトンとして公開される。各フレームワークは ACS を `import` せず `globalThis.__acs` を通じて機能を利用する。`globalThis.__acs` への参照は **Core コンポーネントのみ許可**し、他コンポーネントから直接参照してはならない。型参照は `ACS/ACS.d.ts` からの `import type` のみ許可する
...
ACS（Adlaire Client Services）はサーバ（ASS）との通信を一元的に担うエンジンである。HTTP API 通信・認証管理・ストレージ操作・ファイル操作を責務とする。他フレームワークは ACS を直接 `import` せず、`globalThis.__acs` を通じて機能を利用する。
