...
DI コンテナパターン（サービスロケータを含む）の使用を禁止する。`Container` クラス（APF）は廃止とし、すべてのコードから削除しなければならない。サービス間の依存解決は、ACS については `globalThis.__acs`（§3 参照）、それ以外については `ApplicationFacade` のプロパティ直接参照により行う。
...
| ファイル | 概要 |
|---------|------|
| `APF.Core.ts` | EventBus、Request、Response、および Router を定義する |
