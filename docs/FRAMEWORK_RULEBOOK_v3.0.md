...
AdlairePlatform におけるフレームワークの設計・開発・運用に関する統一規則を定める。
...
- **1 エンジン 5 ファイル原則**
  ...
  - 必須なのは **Core のみ**である。他コンポーネントの有無および役割はエンジンごとに自由に決定できる
- **モジュラーコンポーネント**
...
- **エンジン内協調** — 同一エンジン内でコンポーネントが相互参照する場合は、必ず Core を介さなければならない。Core 以外のコンポーネント同士が直接参照することは禁止する
- **外部依存ゼロ** — サードパーティライブラリおよび外部パッケージへの依存は禁止する。使用できるのは言語標準ライブラリのみである
- **フレームワーク間依存ゼロ** — ACS を含むフレームワーク間の直接 `import` は一切禁止する。サーバとの通信が必要な場合は `globalThis.__acs` を経由しなければならない（§3 参照）
- **共有型ファイル禁止** — `Framework/types.ts` 等の共有型ファイルを作成してはならない。各フレームワークは必要な型を自ら定義するか、ACS が公開する `ACS.d.ts` から `import type` で参照しなければならない

#### ACS アクセス規則

- ACS は `globalThis.__acs` にシングルトンとして公開する
- 各フレームワークは ACS を直接 `import` せず、`globalThis.__acs` を通じて機能を利用しなければならない
- `globalThis.__acs` の参照は **Core コンポーネントのみに許可**する。他コンポーネントからの直接参照は禁止する
...
| 条件 | 許容内容 | 承認 |
|------|---------|------|
| 1 ファイル | 単一ファイルで完結するフレームワークに限り許容する | 承認不要 |
| 2〜5 ファイル | 明確な責務分離の理由がある場合に許容する | 承認不要 |
| 6〜7 ファイル | 明確な責務分離の理由がある場合に許容する | Adlaire Group 承認必須 |
...
| 3 | **リクエスト制御** | リクエストのキャンセル（AbortController）およびタイムアウトを管理する |
| 4 | **障害復旧** | 指数バックオフ付きリトライおよびエラーハンドリングを行う |
...
| 10 | **動的データ取得** | API レスポンスから動的データを取得し、変換する |

#### アーキテクチャ

| # | 責務 | 概要 |
|---|------|------|
| 11 | **ブートストラップ / 初期化** | bootstrap 時に初期化処理を実行し、`globalThis.__acs` を公開する |
| 12 | **グローバルシングルトン** | `globalThis.__acs` としてシングルトンを提供する |
| 13 | **フレームワーク間仲介** | 全フレームワークのサーバ通信を一元的に仲介する |
| 14 | **ルーティング・定数管理** | API エンドポイントを定義し、ストレージディレクトリ定数を管理する |
| 15 | **型定義公開** | `ACS.d.ts` による公開型定義を提供し、`import type` による参照を許可する |
| 16 | **診断システム** | ヘルスチェック、接続状態の監視、およびエラー分類を行う |
...
| **globalThis 経由** | サーバとの通信は必ず `globalThis.__acs` を経由しなければならない |
| **Core 限定参照** | `globalThis.__acs` の参照は各エンジンの Core コンポーネントのみに許可する。他コンポーネントからの直接参照は禁止する |
| **型参照のみ許可** | `ACS.d.ts` からの `import type` のみを許可する |
| **直接 fetch 禁止** | 各フレームワークは `fetch()` を直接呼び出してはならない |
| **初期化順序** | ACS の初期化（`globalThis.__acs` の公開）は bootstrap 処理において最初に実行しなければならない |
...
| **ACS** | Adlaire Client Services | サーバとの通信を一元的に担うクライアントエンジン（§3 参照のこと） | TypeScript | 5 |
| **ACE** | Adlaire Content Engine | コレクション、コンテンツ、およびメタデータの管理を行う | TypeScript | 5 + アセット |
| **AIS** | Adlaire Infrastructure Services | アプリケーションコンテキスト、i18n、API キャッシュ、および診断を提供する | TypeScript | 5 |
| **AP** | Adlaire Platform | 認証、ロギング、セキュリティヘッダー等のミドルウェアとクライアントモジュールを提供する | TypeScript | 5 + JsEngine |
| **ASG** | Adlaire Static Generator | Markdown パース、テンプレートレンダリング、および静的サイトビルドを行う | TypeScript | 5 |
| **ASS** | Adlaire Server System | 認証、ストレージ、ファイル操作、および Git 等のサーバサイドサービスを実装する | PHP | 5 |
| **ADS** | Adlaire Design System | ベーススタイル、コンポーネントスタイル、およびエディタスタイルを定義する | CSS | 3 |
...
> | 言語 / ランタイム | 取得元 |
> |------------------|--------|
> | Deno | `README.md` Prerequisites（`Deno 2.x or later`）、`main.ts` にて `Deno.version.deno` で動的に取得 |
> | TypeScript | `deno.json` の `compilerOptions`（`strict: true`, `noImplicitAny: true`） |
> | PHP | 実行環境（`PHP 8.4.18`）、すべてのファイルで `declare(strict_types=1)` を使用、`match` 式（PHP 8.0+ 機能）を使用 |

#### ランタイム規定

| ルール | 内容 |
|--------|------|
| **Deno 必須** | TypeScript フレームワークの実行、型チェック、リント、フォーマット、およびテストはすべて Deno で行わなければならない |
| **Strict モード必須** | TypeScript のコンパイラオプションとして `strict: true` および `noImplicitAny: true` を有効にしなければならない |
| **ブラウザ向け設定** | クライアントサイドのコード（JsEngine、AEB）は `browser.deno.json` の設定に従い、DOM 型定義（`dom`、`dom.iterable`、`dom.asynciterable`、`esnext`）を使用しなければならない |
| **Node.js 禁止** | Node.js ランタイムおよび npm パッケージを使用してはならない（§2.1「外部依存ゼロ」に準ずる） |
| **PHP バージョン固定** | ASS は PHP 8.4 以上で動作しなければならない。すべてのファイルで `declare(strict_types=1)` を宣言しなければならない |
...
各フレームワークは `Framework/` 配下に接頭辞と同名のディレクトリを持つ。ディレクトリ構成は以下の通りである。
...
| **フラット原則** | コンポーネントファイルはディレクトリ直下に配置しなければならない。ネストは禁止する |
...
| **型定義ファイル** | kebab-case で命名し、拡張子は `.d.ts` とする（例: `browser-types.d.ts`） |
...
| ルール | 内容 |
|--------|------|
| **目的限定** | サブディレクトリはクライアントモジュールやアセット等、コンポーネント以外のファイルを格納する場合にのみ使用する |
| **5 ファイル上限の対象外** | サブディレクトリ内のファイルは §2.1「1 エンジン 5 ファイル原則」の上限には含めない |
