...
DI コンテナパターン（サービスロケータを含む）を依存解決の手段として新規に採用することは禁止する。既存の `Container` クラス（APF）は非推奨とし、新規コードでの使用を禁止する。既存の使用箇所は次回改修時にプロパティ直接参照へ移行しなければならない。サービス間の依存解決は、ACS については `globalThis.__acs`（§3 参照）、それ以外については `ApplicationFacade` のプロパティ直接参照により行う。
...
|--------|---------|------|------|-----------|
| **APF** | Adlaire Platform Foundation | EventBus、Router 等のプラットフォーム基盤 | TypeScript | 5 |
...
| ファイル | 概要 |
|---------|------|
| `APF.Core.ts` | EventBus、Request、Response、および Router を定義する（Container は非推奨：§2.1「DI コンテナパターンの禁止」参照） |
