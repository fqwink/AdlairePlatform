...
| # | 責務 | 概要 |
|---|------|------|
| 1 | **認証** | ログイン・ログアウト・セッション管理・自動認証 |
| 2 | **サーバ通信** | ASS との HTTP API 通信（GET / POST / PUT / DELETE） |
| 3 | **ファイル操作** | ファイルのアップロード・ダウンロード・削除・画像最適化 |
| 4 | **ストレージ** | JSON ファイルの読み書き・削除・存在確認・一覧取得 |
| 5 | **ルーティング・定数管理** | API エンドポイント定義・ストレージディレクトリ定数 |
| 6 | **障害復旧** | 指数バックオフ付きリトライ・エラーハンドリング |
| 7 | **セキュリティ** | CSRF トークン管理・認証ヘッダー・資格情報管理 |
| 8 | **静的コンテンツ配信** | 静的ファイル（HTML / CSS / JS / 画像）の配信 |
| 9 | **動的データ取得** | API レスポンスによる動的データの取得・変換 |
| 10 | **ブートストラップ / 初期化** | bootstrap 時の最初の初期化・`globalThis.__acs` の公開 |
| 11 | **グローバルシングルトン** | `globalThis.__acs` としてシングルトンを提供 |
| 12 | **フレームワーク間仲介** | 全フレームワークのサーバ通信を一元仲介 |
| 13 | **型定義公開** | `ACS.d.ts` による公開型定義の提供・`import type` の許可 |
| 14 | **インターセプター** | リクエスト・レスポンスのパイプライン加工 |
| 15 | **リクエスト制御** | リクエストのキャンセル（AbortController）・タイムアウト管理 |
