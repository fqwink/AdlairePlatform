...
ACS を含むすべてのフレームワーク間において、直接の `import` は一切禁止する。サーバとの通信は `globalThis.__acs` を経由しなければならない（§3 参照）。

**共有型ファイルの作成禁止**

`Framework/types.ts` 等の共有型ファイルを新規に作成してはならない。各フレームワークは、必要な型を自ら定義するか、もしくは ACS が公開する `ACS.d.ts` から `import type` により参照しなければならない。

#### ACS アクセス規則

ACS は `globalThis.__acs` 上にシングルトンとして公開される（§3 参照）。各フレームワークは ACS の実装ファイルを直接 `import` せず、`globalThis.__acs` を通じて ACS の機能を利用しなければならない。

`globalThis.__acs` を参照できるのは、各エンジンの Core コンポーネントのみとする。Core 以外のコンポーネントが `globalThis.__acs` を直接参照することは禁止する。

型の参照については、`ACS/ACS.d.ts` からの `import type` のみを許可する。値を伴う `import` は禁止する。
...
ACS（Adlaire Client Services）は、サーバ（ASS）との通信を一元的に担うクライアントサイドのエンジンである。各フレームワークは ACS の実装ファイルを直接 `import` することなく、`globalThis.__acs` を通じて ACS の機能を利用する。ACS の設計目的は、すべてのフレームワークが個別にサーバ通信を実装することを避け、通信ロジックを単一のエンジンに集約することにある。
...
ACS はブートストラップ時に、以下の構造を `globalThis.__acs` として公開する。各フレームワークは、この 4 つの名前空間を通じて ACS の機能にアクセスする。
...
| **globalThis 経由** | サーバとの通信は、すべて `globalThis.__acs` を経由しなければならない |
| **Core 限定参照** | `globalThis.__acs` を参照できるのは、各エンジンの Core コンポーネントのみとする。Core 以外のコンポーネントが `globalThis.__acs` を直接参照することは禁止する |
