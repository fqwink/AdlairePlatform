...
- **フレームワーク間依存ゼロ** — ACS を含むすべてのフレームワーク間で、直接 `import` を一切禁止する。サーバとの通信は `globalThis.__acs` を経由しなければならない（§3 参照）
- **共有型ファイルの作成禁止** — `Framework/types.ts` 等の共有型ファイルを新規に作成してはならない。各フレームワークは必要な型を自ら定義するか、ACS が公開する `ACS.d.ts` から `import type` で参照しなければならない
...
ACS（Adlaire Client Services）は、サーバ（ASS）との通信を一元的に担うクライアントサイドエンジンである。各フレームワークは ACS の実装を直接 `import` せず、`globalThis.__acs` を通じてその機能を利用する。
...
| 12 | **グローバルシングルトン** | `globalThis.__acs` 上にシングルトンインスタンスを提供し、アプリケーション全体で単一のアクセスポイントを保証する |
...
| **直接 fetch 禁止** | 各フレームワークは `fetch()` を直接呼び出してはならない。HTTP 通信はすべて ACS を経由する |
