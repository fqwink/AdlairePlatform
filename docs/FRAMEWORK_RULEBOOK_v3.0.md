...
### §3.6 グローバルシングルトンと DI コンテナの比較

AdlairePlatform には、依存解決のための 2 つの仕組みが存在する。ACS が採用する **グローバルシングルトン**（`globalThis.__acs`）と、APF が提供する **DI コンテナ**（`Container` クラス）である。両者は目的と適用範囲が異なり、競合するものではなく相補的に機能する。

#### 特性比較

| 観点 | グローバルシングルトン（ACS） | DI コンテナ（APF） |
|------|-------------------------------|---------------------|
| **アクセス手段** | `globalThis.__acs` | `container.make<T>(name)` |
| **依存解決** | 暗黙的（グローバル変数参照） | 明示的（`bind` / `singleton` / `make`） |
| **インスタンス管理** | モジュールキャッシュにより常に単一 | `bind`（毎回生成）と `singleton`（単一）を選択可能 |
| **初期化タイミング** | モジュール評価時（静的） | `make` 呼び出し時（遅延） |
| **import の要否** | 不要（`globalThis` 経由） | 必要（`Container` を import） |
| **型安全性** | `globalThis` の型拡張に依存 | ジェネリクス `make<T>` による型キャスト |
| **循環依存検出** | 構造的に発生しない（import 不要のため） | `resolving` セットによる実行時検出 |
| **テスト時の差替** | `globalThis.__acs` の再代入（§3.5 ルールにより本番では禁止） | `bind` による再登録 |

#### 使い分けの原則

| 原則 | 内容 |
|------|------|
| **ACS はグローバルシングルトン** | ACS はフレームワーク間依存ゼロ（R-2.1-6）を維持しつつ全フレームワークに通信機能を提供する必要がある。`import` を伴わない `globalThis` 経由のアクセスが、この制約を満たす唯一の手段である |
| **エンジン内部は DI コンテナ** | 各エンジンが内部で依存関係を管理する際は、APF の `Container` を使用してよい。DI コンテナは、`bind` による差替やテスト時のモック注入を容易にし、エンジン内部の柔軟性を確保する |
| **混在の禁止** | 同一の依存関係に対して、グローバルシングルトンと DI コンテナの両方を経由するアクセスパスを設けてはならない。アクセス手段は一意でなければならない |
| **DI コンテナへの ACS 登録禁止** | ACS のインスタンスまたはその名前空間を DI コンテナに登録してはならない。ACS へのアクセスは常に `globalThis.__acs` を経由しなければならない（R-3.4-2） |
...
| R-3.6-1 | ACS はグローバルシングルトン | §3.6 比較・使い分け | 有効 |
| R-3.6-2 | エンジン内部は DI コンテナ | §3.6 比較・使い分け | 有効 |
| R-3.6-3 | 混在の禁止 | §3.6 比較・使い分け | 有効 |
| R-3.6-4 | DI コンテナへの ACS 登録禁止 | §3.6 比較・使い分け | 有効 |
