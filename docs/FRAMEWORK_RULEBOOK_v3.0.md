...
エンジン駆動モデルは、AdlairePlatform が採用する独自のフレームワーク設計パターンである。各フレームワークを「エンジン」として抽象化し、構成と依存関係を統一的に管理する。
...
- **モジュラーコンポーネント**
  - 各ファイルは、エンジンを構成する独立した構成要素として機能する
  - 1 つのコンポーネント内に複数のクラスを配置してよい
  - Core 以外のコンポーネントは、固定の役割名や責務に縛られず、エンジン全体の設計意図に沿う範囲で自由に構成できる
- **自己完結**
  - 各エンジンはすべてのコンポーネントを含めて自己完結しなければならない。他のエンジンの存在を前提としてはならない
...
- **共有型ファイルの作成禁止** — `Framework/types.ts` 等の共有型ファイルを新規に作成してはならない。必要な型は各フレームワークが自ら定義するか、ACS が公開する `ACS.d.ts` から `import type` により参照しなければならない

#### ACS アクセス規則

- ACS は `globalThis.__acs` 上にシングルトンとして公開される（§3 参照）
- 各フレームワークは ACS を直接 `import` せず、`globalThis.__acs` を通じて機能を利用しなければならない
- `globalThis.__acs` への参照は **各エンジンの Core コンポーネントのみに許可する**。Core 以外のコンポーネントから直接参照してはならない
...
ACS が担う責務は以下の 16 項目である。各責務はカテゴリごとに分類する。
...
| 3 | **リクエスト制御** | AbortController によるリクエストのキャンセルおよびタイムアウトを管理する |
| 4 | **障害復旧** | 指数バックオフ付きリトライおよびエラーハンドリングを実行する |
...
| **Core 限定参照** | `globalThis.__acs` への参照は各エンジンの Core コンポーネントのみに許可する。Core 以外のコンポーネントから直接参照してはならない |
...
| **初期化順序** | ACS の初期化（`globalThis.__acs` の公開）は、ブートストラップ処理の最初に実行しなければならない |
