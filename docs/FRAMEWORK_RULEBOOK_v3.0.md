...
ACS はブートストラップ時にシングルトンインスタンスを生成し、`globalThis.__acs` として公開する（詳細は §3 を参照）。各フレームワークが ACS の機能を利用する際は、ACS の実装ファイルを直接 `import` するのではなく、この `globalThis.__acs` を通じてアクセスしなければならない。
