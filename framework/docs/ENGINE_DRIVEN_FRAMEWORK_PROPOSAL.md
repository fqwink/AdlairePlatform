...
└── CustomSearchEngine/
    ...
    ├── CustomSearchEngine.php # メインクラス
    ├── config.php            # デフォルト設定
    ├── routes.php            # ルート定義（オプション）
    ├── views/                # ビューファイル（オプション）
    │   └── search.html
    └── assets/               # 静的ファイル（オプション）
        ├── search.js
...
```json
{
  "name": "custom-search",
  "version": "1.0.0",
  "description": "高度な全文検索エンジン",
  "author": "Your Name",
  "license": "MIT",
  "main": "CustomSearchEngine.php",
...
<?php
/**
 * CustomSearchEngine - カスタム検索エンジン（プラグイン例）
 */
class CustomSearchEngine extends BaseEngine {
    ...
    public function register(): void {
        ...
        $router->get('/api/search', [$this, 'handleSearch']);
        
        // コンテナにサービス登録
        $this->container->singleton('search', function() {
            return $this;
        });
    }
     ...
     */
    public function boot(): void {
        $this->log('info', 'Custom search engine booted');
        ...
        // イベントリスナー登録
        $this->on('admin.content.updated', function($data) {
            $this->reindexContent($data);
        });
        
        $this->on('admin.content.deleted', function($data) {
            $this->removeFromIndex($data['slug']);
        });
    }
    ...
    public function shutdown(): void {
        // インデックスを保存
        $this->saveIndex();
        $this->log('info', 'Custom search engine shutdown');
    }
    
    /**
     * 検索リクエスト処理
     */
    public function handleSearch(): Response {
        $query = $_GET['q'] ?? '';
        ...
        $this->emit('searched', ['query' => $query, 'count' => count($results)]);
    ...
    private function search(string $query): array {
        ...
        foreach ($this->index as $slug => $document) {
            $score = 0;
            
            foreach ($terms as $term) {
                if (stripos($document['content'], $term) !== false) {
                    $score++;
                }
            }
            ...
            if ($score > 0) {
                $results[] = [
                    ...
                    'preview' => $this->generatePreview($document['content'], $terms),
                    'score' => $score
                ];
            }
        }
        ...
        // 最大件数制限
        $maxResults = $this->getConfig('max_results', 50);
    ...
    private function loadIndex(): void {
        $indexPath = $this->getConfig('index_path', 'data/search/index');
     ...
     * コンテンツ再インデックス
     */
    private function reindexContent(array $data): void {
        $slug = $data['slug'] ?? '';
    ...
    private function removeFromIndex(string $slug): void {
        unset($this->index[$slug]);
        $this->emit('removed_from_index', ['slug' => $slug]);
    }
...
✅ **Week 1-2: フレームワークコア実装**
- Kernel, Container, EngineManager, EventDispatcher 実装
- BaseEngine 基底クラス作成
- 基本的な単体テスト

✅ **Week 3-4: 統合とテスト**
- index.php へのフレームワーク統合
- 既存エンジンとの互換性確認
- パフォーマンステスト

#### フェーズ2: エンジン移行（8週間）

各エンジンを段階的にBaseEngineベースに移行:

| Week | エンジン | 優先度 | 理由 |
|------|---------|--------|------|
| 5-6 | Logger, AppContext, DiagnosticEngine | 高 | 基盤エンジン |
| 7-8 | CacheEngine, MailerEngine | 高 | 依存少ない |
| 9-10 | AdminEngine, TemplateEngine | 中 | コア機能 |
| 11-12 | ApiEngine, CollectionEngine | 中 | 複雑度高い |
...
<?php
/**
 * compatibility.php - 後方互換レイヤー
 * 
 * 既存コードを破壊せずにフレームワーク化を進める。
 */
...
function emit(string $event, $payload = null): void {
    app()->container()->make('events')->dispatch($event, $payload);
}
...
class AdminEngine {
    ...
    // 後方互換用の静的メソッド
    public static function isLoggedIn(): bool {
        return resolve('admin')->isLoggedIn();
    }
}
```
...
<?php
/**
 * tests/EngineManagerTest.php - エンジンマネージャーのテスト例
 */
