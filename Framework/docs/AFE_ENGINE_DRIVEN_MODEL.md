# Adlaire Platform Foundation (APF) - エンジン駆動モデル 完全解説
...
APFは将来的に**独立したフレームワークリポジトリ**として公開される予定です：
...
APFは**トポロジカルソート（Kahn's Algorithm）** を使用して依存関係を自動解決します：
