						...
						return post({ ap_action: 'apply', zip_url: zip_url, version: version })
							.then(function (data) {
								if (data.error) {
									...
									notify(data.message || 'アップデートが完了しました。ページをリロードします...', 'success');
					...
					.then(function (data) {
						...
						if (data.backups && data.backups.length > 0) {
							...
							data.backups.forEach(function (b) {
								var name = b.name;
								var meta = b.meta || {};
								var date = meta.created_at    ? esc(meta.created_at)           : esc(fmtDate(name));
								var ver  = meta.version_before ? esc(meta.version_before)      : '―';
								var size = meta.size_bytes >= 0 ? fmtSize(meta.size_bytes)     : '―';
								html += '<tr>' +
				...
				post({ ap_action: 'rollback', backup: name })
					.then(function (data) {
						if (data.error) {
							...
							notify(data.message || 'ロールバックが完了しました。ページをリロードします...', 'success');
							setTimeout(function () { location.reload(true); }, 1500);
						}
					})
					.catch(function (err) {
