	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		if (addBtn) {
			addBtn.addEventListener('click', function () {
				...
				const label = (document.getElementById('ap-webhook-label') as HTMLInputElement | null)?.value ?? '';
				const secret = (document.getElementById('ap-webhook-secret') as HTMLInputElement | null)?.value ?? '';
				if (!url.trim()) { alert('URL を入力してください'); return; }
				post('webhook_add', { url: url.trim(), label: label.trim(), secret: secret.trim() }, function (res: APResponse) {
		...
		if (userAddBtn) {
			userAddBtn.addEventListener('click', function () {
				const username = (document.getElementById('ap-user-username') as HTMLInputElement | null)?.value ?? '';
				const password = (document.getElementById('ap-user-password') as HTMLInputElement | null)?.value ?? '';
				const role = (document.getElementById('ap-user-role') as HTMLSelectElement | null)?.value ?? 'editor';
				if (!username.trim() || !password) { alert('ユーザー名とパスワードを入力してください'); return; }
				post('user_add', { username: username.trim(), password: password, role: role }, function (res: APResponse) {
		...
		if (redirectAddBtn) {
			redirectAddBtn.addEventListener('click', function () {
				const from = (document.getElementById('ap-redirect-from') as HTMLInputElement | null)?.value ?? '';
				const to = (document.getElementById('ap-redirect-to') as HTMLInputElement | null)?.value ?? '';
				const code = (document.getElementById('ap-redirect-code') as HTMLSelectElement | null)?.value ?? '301';
				if (!from.trim() || !to.trim()) { alert('旧URLと新URLを入力してください'); return; }
				post('redirect_add', { from: from.trim(), to: to.trim(), code: code }, function (res: APResponse) {
		...
		/* ── 差分デプロイ ZIP ── */
		const deployDiffBtn = document.getElementById('ap-static-deploy-diff') as HTMLButtonElement | null;
		if (deployDiffBtn) {
			deployDiffBtn.addEventListener('click', function () {
				...
				fetch('./', { method: 'POST', headers: { 'X-CSRF-TOKEN': c }, body: fd })
					...
					.catch(function (e: Error) { alert('エラー: ' + e.message); });
			});
		}
	});
})();
