...
(function (): void {
	...
	document.addEventListener('DOMContentLoaded', function (): void {
		const forms: NodeListOf<HTMLFormElement> = document.querySelectorAll<HTMLFormElement>('form.ap-contact');
		for (let i: number = 0; i < forms.length; i++) {
			(function (form: HTMLFormElement): void {
				form.addEventListener('submit', function (e: Event): void {
					e.preventDefault();
					const btn: HTMLButtonElement | null = form.querySelector<HTMLButtonElement>('[type="submit"]');
					...
					fd.forEach(function (v: FormDataEntryValue, k: string): void { params[k] = v as string; });
