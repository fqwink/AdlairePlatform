...
(function (): void {
  ...
  function _emitAEB(event: string, detail: Record<string, unknown>): void {
    if (globalThis.__AP_EventBus__ && typeof globalThis.__AP_EventBus__.emit === "function") {
      try {
        globalThis.__AP_EventBus__.emit(event, detail);
      } catch (_) { /* ignore */ }
    }
  }
...
`;
  ...
  function _uid(): string {
    return "b" + (++_idCounter) + "-" + Math.random().toString(36).slice(2, 7);
  }
  ...
  function _activate(span: HTMLElement): void {
    ...
    try {
      ...
      console.warn("[AP WYSIWYG] execCommand not supported:", (e as Error).message);
    }
    ...
    _blocks.forEach((b: WysiwygBlock): void => {
    ...
    /* 最初のブロックにフォーカス */
    const firstContent = _blocksEl.querySelector('.ap-wy-block-content[contenteditable="true"]') as
      | HTMLElement
      | null;
    if (firstContent) {
      firstContent.focus();
      _setCursorToEnd(firstContent);
    }
    ...
    _blocksEl.addEventListener("dragleave", (): void => {
  ...
  function _serializeBlocks(): string {
    return _blocks.map((b: WysiwygBlock): string => {
      ...
      const alignStyle: string = align && align !== "left" && validAligns.includes(align)
        ? ` style="text-align:${align}"`
        : "";

      switch (b.type) {
        case "paragraph":
          return `<p${alignStyle}>${d.text || "<br>"}</p>`;
        case "heading":
          return `<h${d.level || 2}${alignStyle}>${d.text || ""}</h${d.level || 2}>`;
        case "quote":
          return `<blockquote${alignStyle}>${d.text || ""}</blockquote>`;
        case "code":
          return `<pre><code>${_escHtml(String(d.text || ""))}</code></pre>`;
        case "delimiter":
          return "<hr>";
        case "list": {
  ...
  function _renderListBlock(el: HTMLElement, block: WysiwygBlock): void {
    ...
    list.addEventListener("keydown", (e: KeyboardEvent): void => {
      if (e.key === "Enter") {
        ...
        const li = (anchorNode as HTMLElement)?.closest?.("li") ||
          (anchorNode?.parentElement as HTMLElement)?.closest?.("li");
    ...
    list.addEventListener("input", (): void => {
      _syncListData(list, block);
    });
  ...
  function _syncListData(list: HTMLElement, block: WysiwygBlock): void {
    block.data.items = Array.from(list.querySelectorAll("li")).map((li: Element) =>
      li.innerHTML.trim()
    );
  }
  ...
  function _renderTableBlock(el: HTMLElement, block: WysiwygBlock): void {
    ...
    const _buildTable = (): void => {
      ...
      rows.forEach((row: string[], ri: number): void => {
        ...
        row.forEach((cell: string, ci: number): void => {
          ...
          td.addEventListener("input", (): void => {
            rows[ri][ci] = td.innerHTML.trim();
          });
    ...
    const _tbtn = (label: string, fn: () => void): void => {
      ...
      b.type = "button";
      b.className = "ap-wy-tbl-btn";
      b.textContent = label;
      b.addEventListener("click", (e: MouseEvent): void => {
        e.preventDefault();
        fn();
      });
  ...
  function _renderImageBlock(el: HTMLElement, block: WysiwygBlock): void {
    ...
    if (block.data.src && _isSafeUrl(String(block.data.src))) {
      ...
      if (block.data.width && block.data.width !== "100%") {
        img.style.width = String(block.data.width);
      }
      ...
      ["25%", "50%", "75%", "100%"].forEach((w: string): void => {
        ...
        btn.addEventListener("click", (e: MouseEvent): void => {
          ...
          actions.querySelectorAll(".ap-wy-img-size-btn").forEach((b: Element) =>
      ...
      altInput.style.cssText =
        "width:200px;padding:1px 4px;font-size:11px;background:#333;color:#eee;border:1px solid #555;border-radius:3px;";
      altInput.addEventListener("input", (): void => {
      ...
      caption.addEventListener("input", (): void => {
        block.data.caption = caption.innerHTML.trim();
      });
  ...
  function _renderChecklistBlock(el: HTMLElement, block: WysiwygBlock): void {
    ...
    const items = (block.data.items || [{ text: "", checked: false }]) as Array<
      ...
      items.forEach((item: { text: string; checked: boolean }, idx: number): void => {
        ...
        text.addEventListener("keydown", (e: KeyboardEvent): void => {
          if (e.key === "Enter") {
            ...
            _buildChecklist();
            /* 新しい項目にフォーカス */
            const newText = content.querySelectorAll(".ap-wy-check-text")[idx + 1] as
              | HTMLElement
              | undefined;
          ...
          if (e.key === "Backspace" && (text.textContent || "").trim() === "") {
            ...
            if (idx === 0 && items.length === 1) {
              ...
              items.splice(0, 1);
              _buildChecklist();
              const first = content.querySelectorAll(".ap-wy-check-text")[0] as
                | HTMLElement
                | undefined;
              if (first) {
                first.focus();
                _setCursorToStart(first);
              }
            } else {
              items.splice(idx, 1);
              _buildChecklist();
              const prev = content.querySelectorAll(".ap-wy-check-text")[idx - 1] as
                | HTMLElement
                | undefined;
              if (prev) {
                prev.focus();
                _setCursorToEnd(prev);
              }
            }
          }
          const mod: boolean = e.ctrlKey || e.metaKey;
  ...
  function _attachBlockKeyHandler(
    ...
    target.addEventListener("keydown", (e: KeyboardEvent): void => {
      ...
      /* Enter: ブロック分割（リストは除外） */
      if (
        e.key === "Enter" && !e.shiftKey && block.type !== "list" && block.type !== "code" &&
      ...
      if (e.key === "Backspace" && _isCursorAtStart(target)) {
        ...
        if (idx > 0) {
          ...
          if (
            ["paragraph", "heading", "quote"].includes(prev.type) &&
      ...
      if (e.key === "Delete" && _isCursorAtEnd(target)) {
        ...
        if (idx < _blocks.length - 1) {
          ...
          if (
            ["paragraph", "heading", "quote"].includes(block.type) &&
            ["paragraph", "heading", "quote"].includes(next.type)
          ) {
      ...
      if (e.key === "ArrowDown" && _isCursorAtEnd(target)) {
        ...
        if (idx < _blocks.length - 1) {
          e.preventDefault();
          _focusBlock(_blocks[idx + 1], "start");
        }
        return;
      }
  ...
  function _addBlockAfter(
    afterBlock: WysiwygBlock,
    type: string,
    data: Record<string, unknown>,
  ): WysiwygBlock {
    _saveSnapshot();
  ...
  function _exec(cmd: string, span: HTMLElement, originalHtml: string): void {
    switch (cmd) {
      ...
      case "img": {
        ...
        input.onchange = (): void => {
          if (input.files?.[0]) _insertImageBlock(input.files[0]);
        };
        input.click();
        break;
      }
  ...
  function _toggleInlineCode(): void {
    ...
    try {
      ...
      console.warn("[AP WYSIWYG] inlineCode:", (e as Error).message);
    }
  }
  ...
  function _toggleMarker(): void {
    ...
    try {
      ...
      console.warn("[AP WYSIWYG] marker:", (e as Error).message);
    }
  }
  ...
  function _showLinkInput(): void {
    ...
    while (anchor && (anchor as Element).tagName !== "A" && anchor !== _blocksEl) {
  ...
  function _onSelectionChange(): void {
    ...
    /* 選択がブロックコンテナ内かチェック */
    const range = sel.getRangeAt(0);
    if (!_blocksEl.contains(range.commonAncestorContainer)) {
      _hideInlineToolbar();
      return;
    }
    ...
    if ((blockEl as HTMLElement)?.dataset.type === "code") {
      _hideInlineToolbar();
      return;
    }
  ...
  function _showInlineToolbar(range: Range): void {
    ...
    requestAnimationFrame((): void => {
      ...
      if (tbRect.right > globalThis.innerWidth) {
        _inlineToolbar!.style.left = (globalThis.innerWidth - tbRect.width - 8) + "px";
      }
  ...
  function _showSlashMenu(_target: HTMLElement): void {
    ...
    filtered.forEach((c: SlashCommand, i: number): void => {
      ...
      item.innerHTML =
        `<span class="ap-wy-slash-item-icon">${c.icon}</span><span class="ap-wy-slash-item-label">${c.label}</span>`;
    ...
    if (sel && sel.rangeCount) {
      ...
      requestAnimationFrame((): void => {
        ...
        if (menuRect.bottom > globalThis.innerHeight) {
          top = globalThis.scrollY + rect.top - menuRect.height - 4;
        }
        if (menuRect.right > globalThis.innerWidth) {
  ...
  function _applySlashCmd(): void {
    ...
    /* 現在のブロックの "/" テキストをクリア */
    const el = _getBlockEl(block);
    const content = el?.querySelector('.ap-wy-block-content[contenteditable="true"]') as
      | HTMLElement
      | null;
    ...
    switch (selected.type) {
      ...
      case "checklist":
        ...
        block.data = { items: [{ text: "", checked: false }] };
        _rerenderBlock(block);
        break;
    }
  }
  ...
  function _showTypePopup(block: WysiwygBlock, blockEl: HTMLElement): void {
    ...
    options.forEach((opt: TypePopupOption): void => {
      ...
      if (
        block.type === opt.type && (!opt.extra?.level || block.data.level === opt.extra.level) &&
    ...
    requestAnimationFrame((): void => {
      ...
      if (popRect.bottom > globalThis.innerHeight) {
        top = globalThis.scrollY + handleRect.top - popRect.height - 2;
      }
    ...
    _typePopupKeyHandler = (e: KeyboardEvent): void => {
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        if (_popIdx >= 0) {
          (allItems[_popIdx] as HTMLElement).style.background = "";
          (allItems[_popIdx] as HTMLElement).style.color = "";
        }
    ...
    setTimeout((): void => {
      document.addEventListener("keydown", _typePopupKeyHandler!);
    }, 0);
    ...
    setTimeout((): void => {
      document.addEventListener("mousedown", _typePopupCloseHandler!);
    }, 0);
  }

  function _hideTypePopup(): void {
    if (_typePopupKeyHandler) {
      document.removeEventListener("keydown", _typePopupKeyHandler);
      _typePopupKeyHandler = null;
    }
    if (_typePopupCloseHandler) {
      document.removeEventListener("mousedown", _typePopupCloseHandler);
      _typePopupCloseHandler = null;
    }
    if (_typePopup) {
      _typePopup.remove();
      _typePopup = null;
  ...
  function _startDrag(
    ...
    const _onMove = (e: MouseEvent | TouchEvent): void => {
      const y: number = isTouch
        ? ((e as TouchEvent).touches?.[0]?.clientY ?? startY)
    ...
    document.addEventListener(
      moveEvt,
      _onMove as EventListener,
      isTouch ? { passive: false } : undefined,
    );
  ...
  function _insertImageBlock(file: File): void {
    ...
    const csrf = _getCsrf();
    if (!csrf) {
      console.error("[AP WYSIWYG] CSRF token not found");
      return;
    }
    ...
    fetch("/", {
      ...
      if (focused) {
        ...
        const b: WysiwygBlock = {
          id: _uid(),
          type: "image",
          data: { src: data.url, alt: "", caption: "", width: "100%" },
        };
        _blocks.push(b);
        _blocksEl!.appendChild(_renderBlock(b));
      }
    ...
    panel.addEventListener("keydown", (e: KeyboardEvent): void => {
      ...
      if (e.key === "ArrowDown" || e.key === "ArrowUp") {
        const items = Array.from(
          body.querySelectorAll(".ap-wy-history-item[tabindex]"),
        ) as HTMLElement[];
        if (items.length === 0) return;
  ...
  function _renderRevisionTab(container: HTMLElement, span: HTMLElement): void {
    ...
    _fetchRevisions(fieldname, (revisions: Revision[]): void => {
      ...
      toolbar.style.cssText =
        "padding:6px 10px;border-bottom:1px solid #444;display:flex;gap:8px;flex-wrap:wrap;";
      ...
      cmpBtn.textContent = _diffCompareA
        ? "比較対象A: " + _diffCompareA.file.replace("rev_", "")
        : "2つを比較";
  ...
  function _renderRevisionList(
    container: HTMLElement,
    revisions: Revision[],
    fieldname: string,
    span: HTMLElement,
    _cmpBtn: HTMLButtonElement,
  ): void {
      ...
      .then((data: Record<string, unknown>): void => {
        ...
        if (revisions.length === 0) {
          ...
          noMatch.textContent = query
            ? "「" + query + "」に一致するリビジョンはありません"
            : "リビジョンがありません";
          container.appendChild(noMatch);
          return;
        }
        const cmpBtn = container.querySelector(
          '.ap-wy-history-btn[aria-label="2つのリビジョンを比較"]',
        ) as HTMLButtonElement;
        _renderRevisionList(container, revisions, fieldname, span, cmpBtn);
      })
      .catch((): void => {
        console.warn("revision search failed");
      });
  }
  ...
  function _fetchRevisions(fieldname: string, callback: (revisions: Revision[]) => void): void {
    ...
    fetch("/", {
      ...
      callback((data.revisions || []) as Revision[]);
    })
      .catch((e: Error): void => {
        console.warn("fetchRevisions:", e.message);
        callback([]);
      });
  }

  /* D5: リビジョンコンテンツ取得専用 API */
  function _fetchRevisionContent(
    fieldname: string,
    revFile: string,
    ...
    fetch("/", {
      ...
      .catch((e: Error): void => {
        console.warn("fetchRevisionContent:", e.message);
        callback("");
      });
  }
  ...
  function _pinRevision(fieldname: string, revFile: string, callback?: () => void): void {
    ...
    fetch("/", {
      ...
      .catch((e: Error): void => {
        console.warn("pinRevision:", e.message);
        if (callback) callback();
      });
  }
  ...
  function _renderDiffView(
    ...
    backBtn.addEventListener("click", (): void => {
      if (_historyPanel) {
        const tab = _historyPanel.panel.querySelector(".ap-wy-history-tab.active") as
          | HTMLElement
          | null;
    ...
    while (i < diff.length) {
      ...
      if (d.type !== "equal") {
        ...
        if (eqCount <= FOLD_THRESHOLD) {
          ...
          fold.addEventListener("keydown", (e: KeyboardEvent): void => {
            if (e.key === "Enter") fold.click();
          });
    ...
    if (diff._truncated) {
      ...
      notice.style.cssText =
        "padding:8px 10px;font-size:12px;color:#e2a308;background:rgba(226,163,8,.1);border-radius:4px;margin-top:4px;";
  ...
  function _syncAllBlocks(): void {
    _blocks.forEach((block: WysiwygBlock): void => {
      ...
      if (["paragraph", "heading", "quote"].includes(block.type)) {
        ...
        if (c) {
          block.data.text = c.innerHTML.trim();
          if (block.data.text === "<br>") block.data.text = "";
        }
  ...
  function _sanitizeNode(node: Node): void {
    ...
    Array.from(node.childNodes).forEach((child: ChildNode): void => {
      ...
      Array.from(el.attributes).forEach((attr: Attr): void => {
        ...
        const keep: boolean = (tag === "A" && attr.name === "href") ||
