...
(function () {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		const allButtons: HTMLButtonElement[] = ['ap-static-diff', 'ap-static-full', 'ap-static-clean', 'ap-static-zip']
			.map(function (id) { return document.getElementById(id) as HTMLButtonElement | null; })
			.filter(Boolean) as HTMLButtonElement[];
		let _building = false;

		function showStatus(msg: string): void { statusEl!.textContent = msg; }
		function showResult(html: string): void { resultEl!.innerHTML = html; }
		...
		function renderPages(pages: PageInfo[] | undefined): void {
			...
			pages.forEach(function (p) {
				...
				const label = p.state === 'current' ? '最新' : (p.state === 'outdated' ? '更新必要' : '未生成');
		...
		if (zipBtn) {
			zipBtn.addEventListener('click', function () {
				...
				fetch('./', {
					...
					if (!r.ok) {
						return r.json().then(function (d: { error?: string }) { throw new Error(d.error || 'HTTP ' + r.status); });
					}
					return r.blob();
				}).then(function (blob) {
