...
**設計原則**: 各フレームワークは3〜7ファイル構成で、エンジン駆動アーキテクチャを採用しています。

**Ver.1.8**: 全 7 フレームワーク（APF/ACE/AIS/ASG/AP/AEB/ADS）が Framework/ ディレクトリに統合済み。engines/ は廃止。
...
```
Framework/
├── AP/                           # Platform Controllers (Ver.1.8)
│   ├── AP.Controllers.php       (単一ファイル) - 全 Controller クラス統合
│   ├── AP.Bridge.php            グローバルユーティリティ関数
│   └── JsEngine/                フロントエンド JavaScript モジュール群（17ファイル）
│
...
## Adlaire Platform への統合状況（Ver.1.8）

Ver.1.8 で全フレームワークが Framework/ ディレクトリに統合されました。
旧 engines/ ディレクトリは完全に廃止され、Framework モジュールが直接使用されます。

### インフラ基盤

| ファイル | 役割 |
|---------|------|
| `autoload.php` | `spl_autoload_register` で Framework 名前空間を自動ロード（13プレフィックス） |
| `bootstrap.php` | `Application` クラス（DI Container, HookManager, EventDispatcher） |
| `Framework/AP/AP.Bridge.php` | index.php から分離したグローバルユーティリティ関数 |
| `Framework/AP/JsEngine/aeb-adapter.js` | AEB ES6 モジュール → グローバルスコープブリッジ |
