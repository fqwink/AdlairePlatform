...
(function (): void {
  ...
  function init(): void {
    const input: HTMLInputElement | null = document.getElementById("ap-search-input") as
      | HTMLInputElement
      | null;
    ...
    function loadIndex(callback: (data: SearchIndexItem[]) => void): void {
      ...
      fetch("/search-index.json")
        ...
        .then(function (data: SearchIndexItem[]): void {
          index = data;
          callback(data);
        })
        .catch(function (): void {
          results!.innerHTML = "<p>検索インデックスの読み込みに失敗しました。</p>";
        });
    }
    ...
    function search(query: string, data: SearchIndexItem[]): void {
      ...
      const tokens: string[] = query.toLowerCase().split(/\s+/).filter(
      ...
      scored.sort(function (a: ScoredItem, b: ScoredItem): number {
        return b.score - a.score;
      });
      scored = scored.slice(0, 20);

      if (scored.length === 0) {
        results!.innerHTML = '<p class="ap-search-empty">' +
          escHtml("「" + query + "」に一致する結果はありません。") + "</p>";
        return;
      }
      ...
      for (let k: number = 0; k < scored.length; k++) {
        ...
        html += '<h3><a href="' + escHtml(s.url || "/" + s.slug + "/") + '">' +
          escHtml(s.title || s.slug || "") + "</a></h3>";
    ...
    input.addEventListener("input", function (): void {
      ...
      debounce = setTimeout(function (): void {
        loadIndex(function (data: SearchIndexItem[]): void {
          search((input as HTMLInputElement).value, data);
        });
      }, 300);
    });
  }
