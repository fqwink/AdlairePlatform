...
(function () {
	...
	function fetchApi(action: string, params: Record<string, string> | null, callback: (res: APResponse) => void): void {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		if (createBtn) {
			createBtn.addEventListener('click', function () {
				const name = (document.getElementById('ap-collection-name') as HTMLInputElement | null)?.value ?? '';
				const label = (document.getElementById('ap-collection-label') as HTMLInputElement | null)?.value ?? '';
				if (!name.trim()) { alert('コレクション名を入力してください'); return; }
				post('collection_create', { name: name.trim(), label: label.trim() || name.trim() }, function (res: APResponse) {
					if (res.ok) {
						location.reload();
					} else {
		...
		if (migrateBtn) {
			migrateBtn.addEventListener('click', function () {
				...
				post('collection_migrate', {}, function (res: APResponse) {
					...
					if (res.ok && data) {
						if (resultEl) resultEl.textContent = data.migrated + '件移行しました（全' + data.total + '件）';
		...
		function bindItemEvents(): void {
			...
			/* アイテム削除 */
			document.querySelectorAll<HTMLButtonElement>('.ap-editor-item-delete').forEach(function (btn) {
				btn.addEventListener('click', function (this: HTMLButtonElement, e: Event) {
					e.stopPropagation();
					const slug = this.getAttribute('data-slug');
					if (!confirm('「' + slug + '」を削除しますか？')) return;
					post('collection_item_delete', { collection: currentCollection, slug: slug || '' }, function (res: APResponse) {
						if (res.ok) {
							loadItems(currentCollection);
		...
		if (saveBtn) {
			saveBtn.addEventListener('click', function () {
				...
				const title = (document.getElementById('ap-editor-edit-title') as HTMLInputElement | null)?.value ?? '';
		...
		/* ── 新規アイテム ── */
		const newCreateBtn = document.getElementById('ap-editor-new-create') as HTMLButtonElement | null;
		if (newCreateBtn) {
			newCreateBtn.addEventListener('click', function () {
				const slug = (document.getElementById('ap-editor-new-slug') as HTMLInputElement | null)?.value ?? '';
				const title = (document.getElementById('ap-editor-new-title') as HTMLInputElement | null)?.value ?? '';
				...
				post('collection_item_save', {
					...
					if (res.ok) {
						...
						const titleEl = document.getElementById('ap-editor-new-title') as HTMLInputElement | null;
		...
		function updatePreview(): void {
			...
			const slug = document.getElementById('ap-editor-edit-area')?.getAttribute('data-slug') || 'preview';
			fetchApi('item', { collection: currentCollection, slug: slug }, function (res: APResponse) {
