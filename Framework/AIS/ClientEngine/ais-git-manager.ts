  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (saveBtn) {
      saveBtn.addEventListener("click", function () {
        ...
        post("git_configure", {
          repository:
            (document.getElementById("ap-git-repository") as HTMLInputElement | null)?.value ?? "",
          token: token,
          branch: (document.getElementById("ap-git-branch") as HTMLInputElement | null)?.value ??
            "main",
          content_dir:
            (document.getElementById("ap-git-content-dir") as HTMLInputElement | null)?.value ??
              "content",
          enabled: "1",
          issues_enabled:
            (document.getElementById("ap-git-issues") as HTMLInputElement | null)?.checked
              ? "1"
              : "",
          webhook_secret:
            (document.getElementById("ap-git-webhook-secret") as HTMLInputElement | null)?.value ??
              "",
    ...
    if (testBtn) {
      testBtn.addEventListener("click", function () {
        ...
        post("git_test", {}, function (res: APResponse) {
          if (res.ok) {
            showResult(
              "ap-git-config-result",
              res,
              "✓ 接続成功: " + ((res.name as string) || "") + " (" +
                ((res.default_branch as string) || "main") + ")",
            );
          } else {
    ...
    if (pushBtn) {
      pushBtn.addEventListener("click", function () {
        const msg =
          (document.getElementById("ap-git-commit-msg") as HTMLInputElement | null)?.value ?? "";
        if (!confirm("ローカルのコンテンツを GitHub にプッシュしますか？")) return;
        const resultEl = document.getElementById("ap-git-sync-result");
        if (resultEl) {
          resultEl.style.color = "#718096";
          resultEl.textContent = "Push 中...";
        }
        ...
        post("git_push", { message: msg }, function (res: APResponse) {
          ...
          if (res.ok) {
            const text = "Push 完了: " + ((res.uploaded as number) || 0) + "件アップロード" +
              ((res.commit_sha as string)
                ? " (コミット: " + (res.commit_sha as string).substring(0, 7) + ")"
                : "");
    ...
    if (logBtn) {
      logBtn.addEventListener("click", function () {
        ...
        post("git_log", { limit: "15" }, function (res: APResponse) {
          const commits = (res as APResponse & {
