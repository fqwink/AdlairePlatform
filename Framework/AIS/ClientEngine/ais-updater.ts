...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function notify(msg: string, type: "error" | "success" | "info"): void {
      ...
      el.style.cssText =
        "padding:8px 12px;margin:4px 0;border-radius:4px;font-size:13px;background:#f7fafc;border:1px solid #e2e8f0;" +
        cls;
      el.textContent = msg;
      resultEl.appendChild(el);
      setTimeout(function () {
        if (el.parentNode) el.style.opacity = "0.6";
      }, 8000);
    }
      ...
      checkBtn.addEventListener("click", function (this: HTMLButtonElement) {
        ...
        post({ ap_action: "check" })
          .then(function (data: APResponse) {
            ...
            if (data.error) {
              if (resultEl) {
                resultEl.innerHTML = '<span style="color:#c0392b;">エラー: ' + esc(data.error) +
                  "</span>";
              }
            } else if (data.update_available) {
              if (resultEl) {
                resultEl.innerHTML = '<b style="color:#27ae60;">バージョン ' + esc(data.latest) +
                  " が利用可能です</b>" +
              ...
              if (resultEl) {
                resultEl.innerHTML = '<span style="color:#555;">最新バージョン ' +
                  esc(data.current) + " を使用中です。</span><br>" +
                  '<button id="ap-list-backups" style="margin-top:8px;cursor:pointer;">ロールバック</button>';
              }
            }
          })
    ...
    document.addEventListener("click", function (e: MouseEvent) {
      ...
      if (t.id === "ap-apply-update") {
        ...
        post({ ap_action: "check_env" })
          .then(function (env: APResponse) {
            ...
            const diskMsg = diskFree >= 0 ? "（空き容量: " + fmtSize(diskFree) + "）" : "";
            if (
              !confirm(
                "アップデートを適用します。" + diskMsg +
      ...
      if (t.id === "ap-list-backups") {
        ...
        post({ ap_action: "list_backups" })
          .then(function (data: APResponse) {
            ...
            if (backups && backups.length > 0) {
              ...
              backups.forEach(function (b) {
                ...
                const size = meta.size_bytes != null && meta.size_bytes >= 0
                  ? fmtSize(meta.size_bytes)
                  : "―";
                html += "<tr>" +
                  ...
                  '" style="cursor:pointer;">復元</button> ' +
                  '<button class="ap-delete-backup" data-name="' + esc(name) +
                  '" style="cursor:pointer;color:#c0392b;">削除</button>' +
                  "</td></tr>";
              });
              html += "</table>";
            } else {
          ...
          .catch(function () {
            notify("バックアップ一覧の取得に失敗しました。", "error");
          })
          .finally(function () {
            btn.disabled = false;
          });
        ...
        post({ ap_action: "rollback", backup: name })
          .then(function (data: APResponse) {
            if (data.error) {
              ...
              notify(
                (data.message as string) || "ロールバックが完了しました。ページをリロードします...",
                "success",
              );
              setTimeout(function () {
                location.reload();
      ...
      if (t.classList.contains("ap-delete-backup")) {
        ...
        if (
          !confirm(
            'バックアップ "' + name + '" を削除します。この操作は取り消せません。よろしいですか？',
          )
        ) return;
        btn.disabled = true;
        btn.textContent = "削除中...";
