...
(function () {
  ...
  const post = (action: string, params?: Record<string, string>): Promise<APResponse> =>
    AP.postAction(action, params);
  const h = AP.escHtml;
  ...
  function loadHealth(): void {
    post("diag_health").then((res) => {
      ...
      el.innerHTML = '<span style="color:' + (colors[status] || "#718096") +
        ';font-weight:bold;font-size:16px;">' + (labels[status] || status).toUpperCase() +
        "</span>";

      const diagEl = document.getElementById("ap-diag-health-details");
      if (diagEl && hd.diagnostics) {
        const dd = hd.diagnostics;
        diagEl.innerHTML = "<span>エラー(24h): <strong>" + (dd.errors_24h || 0) +
          "</strong></span>" +
          '<span style="margin-left:16px;">ディスク空き: <strong>' +
          (dd.disk_free_mb != null ? dd.disk_free_mb + " MB" : "-") + "</strong></span>" +
          '<span style="margin-left:16px;">メモリピーク: <strong>' + (dd.memory_peak_mb || "-") +
          " MB</strong></span>";
      }
    }).catch(() => {});
  }
  ...
  function renderSecuritySummary(sec: SecurityData): void {
    ...
    el.innerHTML = items
      .filter((i) => i.count > 0)
      .map((i) =>
        '<span class="ap-diag-badge" style="background:' + i.color + "20;color:" + i.color + ';">' +
        h(i.label) + ": " + i.count + "</span>"
      )
      .join(" ");
  }
  ...
  function renderTimings(timingsData: TimingsData): void {
    ...
    if (entries.length === 0 && !requestTotal) {
      el.innerHTML =
        '<span style="color:#718096;font-size:13px;">計測データなし（次回リクエストで記録されます）</span>';
      return;
    }
    ...
    if (requestTotal != null) {
      html += '<div style="font-size:13px;margin-bottom:8px;">リクエスト合計: <strong>' +
        requestTotal.toFixed(1) + " ms</strong>";
    ...
    entries.forEach(([label, ms]) => {
      ...
      html += '<div class="ap-diag-timing-row">' +
        ...
        '<div class="ap-diag-timing-bar-bg"><div class="ap-diag-timing-bar" style="width:' +
  ...
  function renderTrends(trends: TrendsData): void {
    ...
    days.forEach((d) => {
      ...
      html += '<div class="ap-diag-trend-bar-wrap">' +
        ...
        '<div class="ap-diag-trend-bar" style="height:' + pct.toFixed(0) + "%;background:" + color +
        ';"></div>' +
    ...
    html += '<span style="color:' + (dirColors[dir] || "#718096") + ';font-weight:600;">' +
      (dirLabels[dir] || dir) + "</span>";
  ...
  function renderRecentErrors(errors: ErrorEntry[]): void {
    ...
    let html =
      '<table class="ap-diag-table"><thead><tr><th>日時</th><th>種別</th><th>メッセージ</th><th>ファイル</th></tr></thead><tbody>';
  ...
  function renderRecentLogs(logs: LogEntry[]): void {
    ...
    let html =
      '<table class="ap-diag-table"><thead><tr><th>日時</th><th>カテゴリ</th><th>メッセージ</th></tr></thead><tbody>';
  ...
  function bindEvents(): void {
    /* 有効/無効トグル */
    (document.getElementById("ap-diag-toggle") as HTMLButtonElement | null)?.addEventListener(
      "click",
      function (this: HTMLButtonElement) {
        ...
        post("diag_set_enabled", { enabled: newEnabled ? "1" : "0" }).then((res) => {
          ...
          showResult(
            res.ok ? (newEnabled ? "有効にしました" : "無効にしました") : (res.error || "エラー"),
          );
        });
      },
    );
    ...
    /* レベル変更 */
    (document.getElementById("ap-diag-level") as HTMLSelectElement | null)?.addEventListener(
      "change",
      function (this: HTMLSelectElement) {
        post("diag_set_level", { level: this.value }).then((res) => {
          showResult(
            res.ok ? "レベルを " + this.value + " に変更しました" : (res.error || "エラー"),
          );
        });
      },
    );
    ...
    /* プレビュー */
    (document.getElementById("ap-diag-preview") as HTMLButtonElement | null)?.addEventListener(
      "click",
      function () {
    ...
    /* 今すぐ送信 */
    (document.getElementById("ap-diag-send-now") as HTMLButtonElement | null)?.addEventListener(
      "click",
      function (this: HTMLButtonElement) {
    ...
    /* ログクリア */
    (document.getElementById("ap-diag-clear-logs") as HTMLButtonElement | null)?.addEventListener(
      "click",
      function () {
    ...
    /* 全ログ表示 */
    (document.getElementById("ap-diag-show-all-logs") as HTMLButtonElement | null)
    ...
    /* 初回通知バナーを閉じる */
    (document.getElementById("ap-diag-notice-dismiss") as HTMLButtonElement | null)
      ?.addEventListener("click", function () {
